...
console.log(
  `[info] *** Detected platform: ${currentPlatform}, architecture: ${currentArch}`,
);
const assetBackups = [
...
(async () => {
  ...
  for (const target of targets) {
    ...
    if (intended_platform !== currentPlatform) {
      console.warn(
    ...
    execCmdSync(
      `curl -L ${additionalCurlOptions} -o ${targetDir}/build.tar.gz ${downloadUrl}`,
    );
