...
console.log(`[info] *** Detected platform: ${currentPlatform}, architecture: ${currentArch}`);
const assetBackups = [
...
async function installNodeModuleInTempDirAndCopyToCurrent(packageName, toCopy) {
  ...
  const tempDir = path.join(
    __dirname,
    "tmp",
    `dbsaicle-node_modules-${adjustedName}`,
  );
...
(async () => {
  ...
  for (const target of targets) {
    const [intended_platform, intended_arch] = target.split("-");
    if (
      intended_platform !== currentPlatform
    ) {
      console.warn(
        `[warn] Skipping verification for ${target} as it does not match the current platform/arch (${currentPlatform}-${currentArch})`,
      );
      continue;
    }  
    ...
    const additionalCurlOptions = process.env.CURL_OPTIONS ? process.env.CURL_OPTIONS : "";


    execCmdSync(`curl -L ${additionalCurlOptions} -o ${targetDir}/build.tar.gz ${downloadUrl}`);
    ...
    // Informs the `dbsaicle-binary` of where to look for node_sqlite3.node
