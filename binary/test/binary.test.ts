import { SerializedDbSaicleConfig } from "core";
...
const DBSAICLE_GLOBAL_DIR = path.join(__dirname, "..", ".dbsaicle");
if (fs.existsSync(DBSAICLE_GLOBAL_DIR)) {
  fs.rmSync(DBSAICLE_GLOBAL_DIR, { recursive: true, force: true });
}
fs.mkdirSync(DBSAICLE_GLOBAL_DIR);
...
describe("Test Suite", () => {
  ...
  beforeAll(async () => {
    ...
    const binaryPath = path.join(binaryDir, `dbsaicle-binary${exe}`);
    const expectedItems = [
      `dbsaicle-binary${exe}`,
      `esbuild${exe}`,
      "index.node",
    ...
    if (USE_TCP) {
      ...
      try {
        subprocess = spawn(binaryPath, {
          env: { ...process.env, DBSAICLE_GLOBAL_DIR },
        });
  ...
  it("should create .dbsaicle directory at the specified location with expected files", async () => {
    expect(fs.existsSync(DBSAICLE_GLOBAL_DIR)).toBe(true);
    ...
    const missingFiles = expectedFiles.filter((file) => {
      const filePath = path.join(DBSAICLE_GLOBAL_DIR, file);
  ...
  it("should add and delete a model from config.json", async () => {
    const model: SerializedDbSaicleConfig["models"][number] = {
      title: "Test Model",
      provider: "openai",
      model: "gpt-3.5-turbo",
