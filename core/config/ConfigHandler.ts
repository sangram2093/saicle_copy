import { ConfigResult, ConfigValidationError } from "@dbsaicledev/config-yaml";
...
type ConfigUpdateFunction = (payload: ConfigResult<DbSaicleConfig>) => void;
...
export class ConfigHandler {
  ...
  private async getOrgs(): Promise<{
    ...
    if (isSignedIn) {
      try {
        ...
        errors.push({
          fatal: false,
          message: `Error loading DbSaicle Hub assistants${e instanceof Error ? ":\n" + e.message : ""}`,
        });
      }
    } else {
  ...
  async getLocalProfiles(options: LoadAssistantFilesOptions) {
    /**
     * Users can define as many local agents as they want in a `.dbsaicle/agents` (or previous .dbsaicle/assistants) folder
     */
    ...
    if (options.includeWorkspace) {
      const assistantFiles = await getAllDotDbSaicleDefinitionFiles(
        this.ide,
        options,
        "assistants",
      );
      const agentFiles = await getAllDotDbSaicleDefinitionFiles(
        this.ide,
        options,
        "agents",
      );
  ...
  private notifyConfigListeners(result: ConfigResult<DbSaicleConfig>) {
  ...
  async getSerializedConfig(): Promise<
    ConfigResult<BrowserSerializedDbSaicleConfig>
  > {
    if (!this.currentProfile) {
      return {
        config: undefined,
  ...
  async loadConfig(): Promise<ConfigResult<DbSaicleConfig>> {
    if (!this.currentProfile) {
      return {
        config: undefined,
        errors: undefined,
