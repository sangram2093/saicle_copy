...
export function resolveSerializedConfig(
  ...
  const config = JSONC.parse(content) as unknown as SerializedDbSaicleConfig;
  ...
  return JSONC.parse(content) as unknown as SerializedDbSaicleConfig;
}
...
function loadSerializedConfig(
  workspaceConfigs: DbSaicleRcJson[],
  ideSettings: IdeSettings,
  ideType: IdeType,
  overrideConfigJson: SerializedDbSaicleConfig | undefined,
  ide: IDE,
): ConfigResult<SerializedDbSaicleConfig> {
  let config: SerializedDbSaicleConfig = overrideConfigJson!;
  if (!config) {
    try {
...
async function serializedToIntermediateConfig(
  initial: SerializedDbSaicleConfig,
  ide: IDE,
): Promise<Config> {
...
async function intermediateToFinalConfig({
  ...
  for (const cmd of config.slashCommands ?? []) {
    if ("source" in cmd) {
      dbsaicleConfig.slashCommands.push(cmd);
    } else {
      dbsaicleConfig.slashCommands.push({
        ...cmd,
  ...
  if (orgPolicy?.policy?.allowMcpServers === false) {
    ...
    mcpManager.setConnections(
      (config.experimental?.modelContextProtocolServers ?? []).map(
        (server, index) => ({
          id: `dbsaicle-mcp-server-${index + 1}`,
          name: `MCP Server`,
          ...server,
        }),
      ),
  ...
  const inlineEditModel = getModelByRole(dbsaicleConfig, "inlineEdit")?.title;
  if (inlineEditModel) {
    const match = dbsaicleConfig.modelsByRole.chat.find(
      (m) => m.title === inlineEditModel,
    );
    if (match) {
      dbsaicleConfig.selectedModelByRole.edit = match;
      dbsaicleConfig.modelsByRole.edit = [match]; // The only option if inlineEdit role is set
    } else {
      errors.push({
        fatal: false,
  ...
  if (applyBlockModel) {
    ...
    if (match) {
      dbsaicleConfig.selectedModelByRole.apply = match;
      dbsaicleConfig.modelsByRole.apply = [match]; // The only option if applyCodeBlock role is set
    } else {
      errors.push({
        fatal: false,
  ...
  return { config: dbsaicleConfig, errors };
}
...
async function finalToBrowserConfig(
  final: DbSaicleConfig,
  ide: IDE,
): Promise<BrowserSerializedDbSaicleConfig> {
  return {
...
async function downloadAndInstallEsbuild(ide: IDE) {
  ...
  try {
    ...
    const additionalCurlOptions = process.env.CURL_OPTIONS
      ...
      : "";
    execSync(`curl ${additionalCurlOptions} -fo "${tgzPath}" "${url}"`);
...
async function tryBuildConfigTs() {
  try {
    ...
    console.log(
      `Build error. Please check your ~/.dbsaicle/config.ts file: ${e}`,
    );
  }
}
...
async function loadDbSaicleConfigFromJson(
  ide: IDE,
  ideSettings: IdeSettings,
  ideInfo: IdeInfo,
  uniqueId: string,
...
export {
  ...
  loadDbSaicleConfigFromJson,
  type BrowserSerializedDbSaicleConfig,
};
