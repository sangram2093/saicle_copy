...
export function isLocalDefinitionFile(uri: string): boolean {
  ...
  return (
    normalizedUri.includes(`/.dbsaicle/agents/`) ||
    normalizedUri.includes(`/.dbsaicle/assistants/`)
  );
}
...
export function getDotDbSaicleSubDirs(
  ide: IDE,
  ...
  // Workspace .dbsaicle/<subDirName>
  if (options.includeWorkspace) {
    fullDirs = workspaceDirs.map((dir) =>
      joinPathsToUri(dir, ".dbsaicle", subDirName),
    );
  }
...
/**
 * This method searches in both ~/.dbsaicle and workspace .dbsaicle
 * for all YAML/Markdown files in the specified subdirectory, for example .dbsaicle/assistants or .dbsaicle/prompts
 */
export async function getAllDotDbSaicleDefinitionFiles(
  ide: IDE,
  options: LoadAssistantFilesOptions,
  subDirName: string,
): Promise<{ path: string; content: string }[]> {
  ...
  const fullDirs = getDotDbSaicleSubDirs(
    ide,
    options,
    workspaceDirs,
    subDirName,
