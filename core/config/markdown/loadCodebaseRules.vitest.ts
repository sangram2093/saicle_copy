import { markdownToRule } from "@dbsaicledev/config-yaml";
...
vi.mock("@dbsaicledev/config-yaml", () => ({
...
describe("loadCodebaseRules", () => {
  ...
  const mockFiles = [
    ...
    ".dbsaicle/rules.md", // This should also be loaded
  ];
  ...
  const mockRuleContent: Record<string, string> = {
    ...
    ".dbsaicle/rules.md": "# Global Rules\nFollow project guidelines",
  };
  ...
  it("should load rules from all rules.md files in the workspace", async () => {
    ...
    expect(mockIde.readFile).toHaveBeenCalledWith(".dbsaicle/rules.md");
    ...
    expect(rules).toContainEqual(mockConvertedRules[".dbsaicle/rules.md"]);
