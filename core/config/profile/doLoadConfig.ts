...
import DbSaicleProxyContextProvider from "../../context/providers/DbSaicleProxyContextProvider";
...
import DbSaicleProxy from "../../llm/llms/stubs/DbSaicleProxy";
...
import { getWorkspaceDbSaicleRuleDotFiles } from "../getWorkspaceDbSaicleRuleDotFiles";
import { loadDbSaicleConfigFromJson } from "../load";
...
import { loadDbSaicleConfigFromYaml } from "../yaml/loadYaml";

async function loadRules(ide: IDE) {
  const rules: RuleWithSource[] = [];
  const errors = [];

  // Add rules from .dbsaiclerules files
  const { rules: yamlRules, errors: dbsaicleRulesErrors } =
    await getWorkspaceDbSaicleRuleDotFiles(ide);
  rules.unshift(...yamlRules);
  errors.push(...dbsaicleRulesErrors);

  // Add rules from markdown files in .dbsaicle/rules
...
export default async function doLoadConfig(options: {
  ...
  overrideConfigJson?: SerializedDbSaicleConfig;
  ...
  let newConfig: DbSaicleConfig | undefined;
  ...
  if (overrideConfigYaml || fs.existsSync(configYamlPath)) {
    const result = await loadDbSaicleConfigFromYaml({
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
    ...
    const result = await loadDbSaicleConfigFromJson(
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
  ...
  const proxyContextProvider = newConfig.contextProviders?.find(
    (cp) => cp.description.title === "dbsaicle-proxy",
  );
  if (proxyContextProvider) {
    (proxyContextProvider as DbSaicleProxyContextProvider).workOsAccessToken =
      workOsAccessToken;
  }
  ...
  // TODO: Remove DbSaicle team member check once Sentry is ready for all users
  let userEmail: string | undefined;
  try {
    // Access the session info to get user email for DbSaicle team member check
  ...
  const useOnPremProxy =
    controlPlane?.useDbSaicleForTeamsProxy === false && controlPlane?.proxyUrl;
...
): Promise<DbSaicleConfig> {
  Object.keys(config.modelsByRole).forEach((key) => {
    config.modelsByRole[key as ModelRole].forEach((model) => {
      if (model.providerName === "dbsaicle-proxy") {
        (model as DbSaicleProxy).controlPlaneProxyInfo = info;
      }
    });
  });
