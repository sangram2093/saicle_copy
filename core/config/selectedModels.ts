...
import { DbSaicleConfig, ILLM } from "..";
...
export function rectifySelectedModelsFromGlobalContext(
  ...
  for (const role of roles) {
    ...
    if (currentSelection) {
      const match = dbsaicleConfig.modelsByRole[role].find(
    ...
    if (!newModel && dbsaicleConfig.modelsByRole[role].length > 0) {
      newModel = dbsaicleConfig.modelsByRole[role][0];
    }
