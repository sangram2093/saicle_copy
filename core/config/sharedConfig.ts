...
export const sharedConfigSchema = z
  .object({
    ...
    // `experimental` in `DbSaicleConfig`
    ...
    // `tabAutocompleteOptions` in `DbSaicleConfig`
...
// - SerializedDbSaicleConfig (config.json)
// - Config ("intermediate") - passed to config.ts
// - DbSaicleConfig
// - BrowserSerializedDbSaicleConfig (final converted to be passed to GUI)
...
//   Which don't exist on SerializedDbSaicleConfig/Config types, so must be added after the fact
export function modifyAnyConfigWithSharedConfig<
  T extends
    ...
    | Config
    | SerializedDbSaicleConfig,
>(dbsaicleConfig: T, sharedConfig: SharedConfigSchema): T {
  const configCopy = { ...dbsaicleConfig };
