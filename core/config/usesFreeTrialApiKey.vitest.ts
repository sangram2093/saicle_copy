...
import { BrowserSerializedDbSaicleConfig } from "..";
...
describe("usesFreeTrialApiKey", () => {
  ...
  let SecretType: typeof import("@dbsaicledev/config-yaml").SecretType;

  beforeEach(async () => {
    mockDecodeSecretLocation.mockReset();
    usesFreeTrialApiKey = (await import("./usesFreeTrialApiKey"))
      .usesFreeTrialApiKey;
    SecretType = (await import("@dbsaicledev/config-yaml")).SecretType;
  });
  ...
  test("usesFreeTrialApiKey should return false when no models have apiKeyLocation", () => {
    const config: BrowserSerializedDbSaicleConfig = {
      modelsByRole: {
        chat: [
          {
            title: "Model 1",
