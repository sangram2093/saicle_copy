import { BlockType, RULE_FILE_EXTENSION } from "@dbsaicledev/config-yaml";
...
describe("getFileContent", () => {
  ...
  test("generates correct YAML for different block types", () => {
    ...
    expect(docsResult).toContain("startUrl: https://docs.dbsaicle.dev");
...
describe("findAvailableFilename", () => {
  test("returns base filename when it doesn't exist", async () => {
    ...
    const result = await findAvailableFilename(
      "/workspace/.dbsaicle/models",
      "models",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.dbsaicle/models/new-model.yaml");
  });

  test("returns filename with counter when base exists", async () => {
    const mockFileExists = async (uri: string) => {
      return uri === "/workspace/.dbsaicle/models/new-model.yaml";
    };
    ...
    expect(result).toBe("/workspace/.dbsaicle/models/new-model-1.yaml");
  });

  test("increments counter until available filename is found", async () => {
    const existingFiles = new Set([
      "/workspace/.dbsaicle/context/new-context.yaml",
      "/workspace/.dbsaicle/context/new-context-1.yaml",
      "/workspace/.dbsaicle/context/new-context-2.yaml",
    ]);
    ...
    const result = await findAvailableFilename(
      "/workspace/.dbsaicle/context",
      "context",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.dbsaicle/context/new-context-3.yaml");
  });
  ...
  test("handles rules markdown files with counter", async () => {
    const existingFiles = new Set([
      `/workspace/.dbsaicle/rules/new-rule.${RULE_FILE_EXTENSION}`,
      `/workspace/.dbsaicle/rules/new-rule-1.${RULE_FILE_EXTENSION}`,
    ]);
    ...
    const result = await findAvailableFilename(
      "/workspace/.dbsaicle/rules",
      "rules",
      mockFileExists,
    );

    expect(result).toBe(
      `/workspace/.dbsaicle/rules/new-rule-2.${RULE_FILE_EXTENSION}`,
    );
  });
  ...
  test("handles large counter values", async () => {
    const existingFiles = new Set(
      Array.from({ length: 100 }, (_, i) =>
        i === 0
          ? "/workspace/.dbsaicle/prompts/new-prompt.yaml"
          : `/workspace/.dbsaicle/prompts/new-prompt-${i}.yaml`,
      ),
    );
    ...
    const result = await findAvailableFilename(
      "/workspace/.dbsaicle/prompts",
      "prompts",
      mockFileExists,
    );

    expect(result).toBe("/workspace/.dbsaicle/prompts/new-prompt-100.yaml");
  });
});
