...
import { getDbSaicleDotEnv } from "../../util/paths";
...
export class LocalPlatformClient implements PlatformClient {
  ...
  /**
   * searches for the first valid secret file in order of ~/.dbsaicle/.env, <workspace>/.dbsaicle/.env, <workspace>/.env
   */
  ...
  private findSecretInLocalEnvFile(fqsn: FQSN): string | undefined {
    try {
      const dotEnv = getDbSaicleDotEnv();
      return dotEnv[fqsn.secretName];
    } catch (error) {
      console.warn(
        `Error reading ~/.dbsaicle/.env file: ${error instanceof Error ? error.message : String(error)}`,
      );
      return undefined;
    }
  }
    ...
    try {
      ...
      for (const folder of workspaceDirs) {
        ...
        try {
          ...
          // DbSaicle to next workspace folder
        }
      }
