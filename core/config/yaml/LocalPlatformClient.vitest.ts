import { FQSN, SecretResult, SecretType } from "@dbsaicledev/config-yaml";
import {
  afterEach,
  beforeEach,
  describe,
...
describe("LocalPlatformClient", () => {
  const testFQSN: FQSN = {
    ...
    secretName: "TEST_DBSAICLE_SECRET_KEY",
  };
  ...
  beforeEach(
    ...
    () => {
      ...
      envKeyValues = {
        TEST_DBSAICLE_SECRET_KEY: secretValue,
  ...
  describe("searches for secrets in local .env files", () => {
    let getDbSaicleDotEnv: Mock;
    beforeEach(async () => {
      const utilPaths = await import("../../util/paths");
      getDbSaicleDotEnv = vi.fn(() => envKeyValues);
      utilPaths.getDbSaicleDotEnv = getDbSaicleDotEnv;
    });

    test("should be able to get secrets from ~/.dbsaicle/.env files", async () => {
      const localPlatformClient = new LocalPlatformClient(
        null,
        testControlPlaneClient,
        testIde,
      ...
      const resolvedFQSNs = await localPlatformClient.resolveFQSNs([testFQSN]);
      expect(getDbSaicleDotEnv).toHaveBeenCalled();
  ...
  describe("should be able to get secrets from workspace .env files", () => {
    test("should get secrets from <workspace>/.dbsaicle/.env and <workspace>/.env", async () => {
      ...
      const randomValueForDbSaicleDirDotEnv =
        "dbsaicle-dir-" + Math.floor(Math.random() * 100);
      const randomValueForWorkspaceDotEnv =
        "dotenv-" + Math.floor(Math.random() * 100);

      testIde.readFile = vi.fn(async (fileUri: string) => {
        // fileUri should contain .dbsaicle/.env and not .env
        if (fileUri.match(/.*\.dbsaicle\/\.env.*/gi)?.length) {
          return (
            envKeyValuesString.split("\n")[0] + randomValueForDbSaicleDirDotEnv
          );
        }
        // filUri should contain .env and not .dbsaicle/.env
        else if (fileUri.match(/.*(?<!\.dbsaicle\/)\.env.*/gi)?.length) {
          return (
      ...
      const dbsaicleDirSecretValue = (
        resolvedFQSNs[0] as SecretResult & { value: unknown }
      )?.value;
      const dotEnvSecretValue = (
        resolvedFQSNs[1] as SecretResult & { value: unknown }
      )?.value;
      expect(dbsaicleDirSecretValue).toContain(secretValue);
      expect(dbsaicleDirSecretValue).toContain(randomValueForDbSaicleDirDotEnv);
      expect(dotEnvSecretValue).toContain(secretValue + "-workspace");
      expect(dotEnvSecretValue).toContain(randomValueForWorkspaceDotEnv);
    });

    test("should first get secrets from <workspace>/.dbsaicle/.env and then <workspace>/.env", async () => {
      const originalIdeFileExists = testIde.fileExists;
      testIde.fileExists = vi.fn(async (fileUri: string) =>
        fileUri.includes(".env") ? true : originalIdeFileExists(fileUri),
      );
      ...
      // we check that workspace <workspace>.dbsaicle/.env does not override the <workspace>/.env secret
      expect(
        (resolvedFQSNs[0] as SecretResult & { value: unknown })?.value,
      ).toContain(randomValueForDbSaicleDirDotEnv);
      expect(
        (resolvedFQSNs[0] as SecretResult & { value: unknown })?.value,
  ...
  describe("should be able to get secrets from process.env", () => {
    ...
    beforeEach(async () => {
      ...
      const utilPaths = await import("../../util/paths");
      utilPaths.getDbSaicleDotEnv = vi.fn(() => ({}));
    ...
    test("should prioritize local ~/.dbsaicle/.env file over process.env", async () => {
      const localEnvFileValue = "secret-from-local-dot-dbsaicle-env";
      const utilPaths = await import("../../util/paths");
      utilPaths.getDbSaicleDotEnv = vi.fn(() => ({
    ...
    test("should prioritize workspace .env files over process.env", async () => {
      const workspaceDbSaicleEnvValue = "secret-from-workspace-dbsaicle-env";
      testIde.fileExists = vi.fn(async (fileUri: string) =>
        // Only mock existence for <workspace>/.dbsaicle/.env
        fileUri.includes(".dbsaicle/.env"),
      );
      testIde.readFile = vi.fn(async (fileUri: string) => {
        if (fileUri.includes(".dbsaicle/.env")) {
          return `${testFQSN.secretName}=${workspaceDbSaicleEnvValue}`;
        }
        return "";
      });
