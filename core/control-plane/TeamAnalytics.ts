...
import DbSaicleProxyAnalyticsProvider from "./analytics/DbSaicleProxyAnalyticsProvider.js";
import {
...
function createAnalyticsProvider(
  ...
  switch (config.provider) {
    ...
    case "dbsaicle-proxy":
      return new DbSaicleProxyAnalyticsProvider();
    default:
      return undefined;
  }
}
...
export class TeamAnalytics {
  ...
  static async setup(
    ...
    if (config.provider === "dbsaicle-proxy") {
      (
        TeamAnalytics.provider as DbSaicleProxyAnalyticsProvider
      ).controlPlaneClient = controlPlaneClient;
    }
  }
