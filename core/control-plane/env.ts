...
export const EXTENSION_NAME = "dbsaicle";
...
const PRODUCTION_HUB_ENV: ControlPlaneEnv = {
  DEFAULT_CONTROL_PLANE_PROXY_URL: "https://api.dbsaicle.dev/",
  CONTROL_PLANE_URL: "https://api.dbsaicle.dev/",
  AUTH_TYPE: AuthType.WorkOsProd,
  WORKOS_CLIENT_ID: WORKOS_CLIENT_ID_PRODUCTION,
  APP_URL: "https://hub.dbsaicle.dev/",
};

const STAGING_ENV: ControlPlaneEnv = {
  DEFAULT_CONTROL_PLANE_PROXY_URL: "https://api.dbsaicle-stage.tools/",
  CONTROL_PLANE_URL: "https://api.dbsaicle-stage.tools/",
  AUTH_TYPE: AuthType.WorkOsStaging,
  WORKOS_CLIENT_ID: WORKOS_CLIENT_ID_STAGING,
  APP_URL: "https://hub.dbsaicle-stage.tools/",
};

const TEST_ENV: ControlPlaneEnv = {
  DEFAULT_CONTROL_PLANE_PROXY_URL: "https://api-test.dbsaicle.dev/",
  CONTROL_PLANE_URL: "https://api-test.dbsaicle.dev/",
  AUTH_TYPE: AuthType.WorkOsStaging,
  WORKOS_CLIENT_ID: WORKOS_CLIENT_ID_STAGING,
  APP_URL: "https://app-test.dbsaicle.dev/",
};
...
export async function enableHubDbSaicleDev() {
  return true;
}
...
export async function getControlPlaneEnv(
  ...
  return getControlPlaneEnvSync(ideSettings.dbsaicleTestEnvironment);
}

export function getControlPlaneEnvSync(
  ideTestEnvironment: IdeSettings["dbsaicleTestEnvironment"],
): ControlPlaneEnv {
  // MDM override
...
export async function useHub(
  ...
  const ideSettings = await ideSettingsPromise;
  return ideSettings.dbsaicleTestEnvironment !== "none";
}
