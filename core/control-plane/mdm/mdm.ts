...
const DBSAICLE_PUBLIC_KEY = `-----BEGIN PUBLIC KEY-----
...
export function validateLicenseKey(licenseKey: string): {
  ...
  try {
    ...
    const isValid = verify.verify(DBSAICLE_PUBLIC_KEY, signature, "base64");
...
const MACOS_MDM_PATHS = [
  ...
  "/Library/Managed Preferences/dev.dbsaicle.app.plist",
  // User-specific MDM plist
  path.join(os.homedir(), "Library/Managed Preferences/dev.dbsaicle.app.plist"),
];
...
function readMdmKeysWindows(): MdmKeys | undefined {
  try {
    ...
    const regPath = "HKLM\\Software\\DbSaicle\\MDM";
    const userRegPath = "HKCU\\Software\\DbSaicle\\MDM";
