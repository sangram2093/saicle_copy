...
import { SYSTEM_PROMPT_DOT_FILE } from "./config/getWorkspaceDbSaicleRuleDotFiles";
...
import { BLOCK_TYPES, ConfigYaml } from "@dbsaicledev/config-yaml";
...
export class Core {
  ...
  constructor(
    ...
    try {
      // Ensure .dbsaicle directory is created
      ...
      (config) => ({
        ...
        models: [
          ...(config.models ?? []),
          {
            ...
            wif_json_path: model.wif_json_path,
            email: model.email,
            kannon_id: model.kannon_id,
            dbllm_apikey: model.dbllm_apikey,
            dbllm_urlendpoint: model.dbllm_urlendpoint,
  ...
  private async handleFilesChanged({
    ...
    if (data?.uris?.length) {
      ...
      for (const uri of data.uris) {
        ...
        if (
          uri.endsWith(".dbsaiclerc.json") ||
          uri.endsWith(".prompt") ||
          uri.endsWith(SYSTEM_PROMPT_DOT_FILE) ||
          (uri.includes(".dbsaicle") &&
            (uri.endsWith(".yaml") || uri.endsWith("yml"))) ||
          BLOCK_TYPES.some((blockType) => uri.includes(`.dbsaicle/${blockType}`))
        ) {
          await this.configHandler.reloadConfig(
            "Config-related file updated: dbsaiclerc, prompt, local block, etc",
          );
  ...
  private getContextItems = async (
    ...
    try {
      ...
      if (e instanceof Error) {
        ...
        //           "https://docs.dbsaicle.dev/customize/model-roles/embeddings",
        //         );
        //       }
        //     });
        // }
