...
import { DbSaicleConfig, IdeInfo, IdeSettings } from "../index.js";
...
export class DataLogger {
  ...
  async addBaseValues(
    ...
    if ("userAgent" in zodSchema.shape) {
      newBody.userAgent = ideInfo
        ? `${ideInfo.name}/${ideInfo.version} (DbSaicle/${ideInfo.extensionVersion})`
        : "Unknown/Unknown (DbSaicle/Unknown)";
    }
  ...
  async logToOneDestination(
    dataConfig: NonNullable<DbSaicleConfig["data"]>[number],
    event: DevDataLogEvent,
  ) {
    try {
      if (!dataConfig) {
      ...
      if (uriComponents.scheme === "https" || uriComponents.scheme === "http") {
        ...
        // If an API key is provided, use it, otherwise use the DbSaicle access token
