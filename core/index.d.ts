...
export interface DbSaicleError {
  title: string;
  message: string;
}
...
export interface AssistantChatMessage {
  ...
  toolCalls?: ToolCallDelta[];
  usage?: Usage;
  /** Optional model provided finish reason (e.g. "STOP", "MAX_TOKENS", provider specific values) */
  finishReason?: string;
  /** Convenience flag to indicate if the response was truncated set when finishReason == "MAX_TOKENS" so callers can easily detect truncation  */
  truncated?: boolean;
}
...
export interface IdeSettings {
  ...
  userToken: string;
  dbsaicleTestEnvironment: "none" | "production" | "staging" | "local";
...
export interface SlashCommand extends SlashCommandDescription {
  run: (sdk: DbSaicleSDK) => AsyncGenerator<string | undefined>;
}

export interface SlashCommandWithSource extends SlashCommandDescription {
  run?: (sdk: DbSaicleSDK) => AsyncGenerator<string | undefined>; // Optional - only needed for legacy
...
// export type DbSaicleConfigSource = "local-yaml" | "local-json" | "hub-assistant" | "hub"
...
export interface DbSaicleUIConfig {
...
// config.json
export interface SerializedDbSaicleConfig {
  env?: string[];
...
export type DbSaicleRcJson = Partial<SerializedDbSaicleConfig> & {
  mergeBehavior: ConfigMergeType;
};

// config.ts - give users simplified interfaces
export interface Config {
  /** If set to true, DbSaicle will collect anonymous usage data to improve the product. If set to false, we will collect nothing. Read here to learn more: https://docs.dbsaicle.dev/telemetry */
  allowAnonymousTelemetry?: boolean;
   ...
   * DbSaicle will do the rest of the work to construct prompt templates, handle context items, prune context, etc.
   */
   ...
   * A CustomContextProvider requires you only to define a title and getContextItems function. When you type '@title <query>', DbSaicle will call `getContextItems(query)`.
   */
  contextProviders?: (CustomContextProvider | ContextProviderWithParams)[];
  /** If set to true, DbSaicle will not index your codebase for retrieval */
  disableIndexing?: boolean;
  /** If set to true, DbSaicle will not make extra requests to the LLM to generate a summary title of each session. */
  disableSessionTitles?: boolean;
  /** An optional token to identify a user. Not used by DbSaicle unless you write custom coniguration that requires such a token */
  userToken?: string;
  /** The provider used to calculate embeddings. If left empty, DbSaicle will use transformers.js to calculate the embeddings with all-MiniLM-L6-v2 */
  embeddingsProvider?: EmbeddingsProviderDescription | ILLM;
  /** The model that DbSaicle will use for tab autocompletions. */
...
// in the actual DbSaicle source code
export interface DbSaicleConfig {
  allowAnonymousTelemetry?: boolean;
  // systemMessage?: string;
  completionOptions?: BaseCompletionOptions;
  requestOptions?: RequestOptions;
  ...
  data?: DataDestination[];
  // Optional Jira configuration from config.yaml
  jira?: {
    domain?: string;
    apiToken?: string;
    authEmail?: string;
