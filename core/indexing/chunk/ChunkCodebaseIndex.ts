...
import { IDbSaicleServerClient } from "../../dbsaicleServer/interface.js";
...
export class ChunkCodebaseIndex implements CodebaseIndex {
  ...
  constructor(
    ...
    private readonly dbsaicleServerClient: IDbSaicleServerClient,
  ...
  async *update(
    ...
    if (this.dbsaicleServerClient.connected) {
      try {
        const keys = results.compute.map(({ cacheKey }) => cacheKey);
        const resp = await this.dbsaicleServerClient.getFromIndexCache(
          keys,
          "chunks",
          repoName,
        );
