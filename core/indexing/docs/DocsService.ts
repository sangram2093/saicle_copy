...
const siteIndexingConfigsAreEqual = (
  siteConfig1: SiteIndexingConfig,
  siteConfig2: SiteIndexingConfig,
  contConfig1: DbSaicleConfig | undefined,
  contConfig2: DbSaicleConfig,
) => {
  return (
...
export default class DocsService {
  ...
  async indexAndAdd(
    ...
    try {
      await provider.embed(["dbsaicle-test-run"]);
    } catch (e) {
  ...
  private async syncDocs(
    oldConfig: DbSaicleConfig | undefined,
    newConfig: DbSaicleConfig,
    ...
    try {
      ...
      for (const doc of newConfigDocs) {
        ...
        if (currentIndexedDoc) {
          ...
          // TODO: Changes to the docs config made while DbSaicle isn't running won't be caught
          if (
            oldConfigDoc &&
            ...
            if (forceReindex) {
              ...
              // This is a temperary fix to catch the changes to the docs config that were made when DbSaicle isn't running
