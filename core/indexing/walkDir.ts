...
import { getGlobalDbSaicleIgArray } from "./dbsaicleignore";
...
export async function getIgnoreContext(
  ...
  // These are done separately so that .dbsaicleignore can override .gitignore
  const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore");
  const dbsaicleIgnoreFile = dirFiles.find(
    (name) => name === ".dbsaicleignore",
  );
  ...
  const getDbSaicleIgnorePatterns = async () => {
    if (dbsaicleIgnoreFile) {
      const contents = await ide.readFile(`${currentDir}/.dbsaicleignore`);
      return gitIgArrayFromFile(contents);
    }
    return [];
  };
  ...
  const ignoreContext = ignore()
    ...
    .add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .dbsaicleignore - this is combined for speed
    .add(ignoreArrays[1]); // local .dbsaicleignore
