export const DEFAULT_SYSTEM_MESSAGES_URL =
  "https://github.com/dbsaicledev/continue/blob/main/core/llm/defaultSystemMessages.ts";
...
const BRIEF_COMPLETE_CODE_INSTRUCTIONS = `You must ensure usage of complete context and not omit any relevant parts.`;
...
<important_rules>
  ...
  You MUST call one and only obne tool at a time. Even if you need tp use multiple tools, then call them one by one. Only after the success or failure of current tool is confirmed then you can call the next tool. 

${CODEBLOCK_FORMATTING_INSTRUCTIONS}

${BRIEF_COMPLETE_CODE_INSTRUCTIONS}
