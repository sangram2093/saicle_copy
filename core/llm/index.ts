import { ModelRole } from "@dbsaicledev/config-yaml";
import { fetchwithRequestOptions } from "@dbsaicledev/fetch";
import { findLlmInfo } from "@dbsaicledev/llm-info";
import {
  BaseLlmApi,
  ChatCompletionCreateParams,
  constructLlmApi,
} from "@dbsaicledev/openai-adapters";
...
export abstract class BaseLLM implements ILLM {
  ...
  /**
   * This exists because for the dbsaicle-proxy, sometimes we want to get the value of the underlying provider that is used on the server
  ...
  constructor(_options: LLMOptions) {
    ...
    // Use @dbsaicledev/llm-info package to autodetect certain parameters
    const modelSearchString =
      this.providerName === "dbsaicle-proxy"
    ...
    this.dbllm_apikey = options.dbllm_apikey;
    this.kannon_id = options.kannon_id;
    this.dbllm_urlendpoint = options.dbllm_urlendpoint;
    this.data_classification = options.data_classification;
  ...
  get contextLength() {
    ...
    if (llmInfo?.contextLength) {
      this._contextLength = llmInfo.contextLength;
      console.log(
        `contextLength set to ${this._contextLength} for model ${this.model}`,
      );
    }
  ...
  private _modifyCompletionOptions(
    ...
    // See these threads:
    // - https://github.com/dbsaicledev/dbsaicle/issues/3698
