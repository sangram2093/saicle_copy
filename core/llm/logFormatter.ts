...
export class LLMLogFormatter {
  ...
  private logFragment(
    ...
    if (
      ...
      dbsaicleAt = startAt + this.wrapWidth - this.openLineChars;

      // Look for a better line-breaking point at whitespace
      const searchBackwardLimit = Math.max(startAt, dbsaicleAt - 20); // Don't look back too far
      for (let i = dbsaicleAt; i > searchBackwardLimit; i--) {
        if (/\s/.test(fragment.charAt(i))) {
          dbsaicleAt = i + 1; // Break after the whitespace
          break;
        }
      }
      ...
      fragment = fragment.substring(startAt, dbsaicleAt).trimEnd();
      joinAfter = false;
    } else if (startAt > 0) {
