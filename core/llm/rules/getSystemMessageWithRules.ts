...
/**
 * Checks if a rule is a root-level rule (.dbsaicle directory or no file path)
 */
const isRootLevelRule = (rule: RuleWithSource): boolean => {
  return !rule.ruleFile || rule.ruleFile.includes(".dbsaicle/"); // ruleFile path is absolute - hence we need to check for it in between
};
...
export const shouldApplyRule = (
  ...
  // Check if this is a root-level rule (in .dbsaicle directory or no file path)
