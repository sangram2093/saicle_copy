import { createRuleMarkdown } from "@dbsaicledev/config-yaml";
...
export const createRuleBlockImpl: ToolImpl = async (args, extras) => {
  ...
  const [localDbSaicleDir] = await extras.ide.getWorkspaceDirs();
  const ruleFilePath = createRuleFilePath(localDbSaicleDir, name);
