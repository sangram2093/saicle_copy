  ...
  readFile(fileUri: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(filepath, "utf8", (err, contents) => {
        ...
        try {
          //normalize content:
          // 1. Striop UTG-8 BOM if present
          if (contents && contents.charCodeAt(0) === 0xfeff) {
            contents = contents.slice(1);
          }
          // 2. If file appears to contain a single JSON-style quoted string that actually encodes multiline code
          // e.g. "function foo() {\n return 1; \n}") unquote and unescape
          // Heuristic: starts and ends with '"', contains escape sequences (\\n/\\r/\\t/\\"/\\\\)
          // and no real newlines inside ( to avoid double-decoding real multiline files).
          if (
            ...
            const inner = contents.slice(1, -1);
            const hasEscapes = /\\n|\\r|\\t|\\"|\\\\/.test(inner);
            const hasRealNewLine = /\n/.test(inner);
            if (hasEscapes && !hasRealNewLine) {
              try {
          ...
          // if normalization fails then fall back to raw contents.
          // (intentionally swallow error to avoid breaking file read.)
        }
        resolve(contents);
      });
    });
