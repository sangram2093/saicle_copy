...
import { ConfigYaml, DevEventName } from "@dbsaicledev/config-yaml";
import * as JSONC from "comment-json";
import dotenv from "dotenv";

import { IdeType, SerializedDbSaicleConfig } from "../";
import { defaultConfig } from "../config/default";
import Types from "../config/types";

dotenv.config();

const DBSAICLE_GLOBAL_DIR = (() => {
  const configPath = process.env.DBSAICLE_GLOBAL_DIR;
  if (configPath) {
  ...
  return path.join(os.homedir(), ".dbsaicle");
})();
...
export function getChromiumPath(): string {
  return path.join(getDbSaicleUtilsPath(), ".chromium-browser-snapshots");
}

export function getDbSaicleUtilsPath(): string {
  const utilsPath = path.join(getDbSaicleGlobalPath(), ".utils");
...
export function getGlobalDbSaicleIgnorePath(): string {
  const dbsaicleIgnorePath = path.join(
    getDbSaicleGlobalPath(),
    ".dbsaicleignore",
  );
  if (!fs.existsSync(dbsaicleIgnorePath)) {
    fs.writeFileSync(dbsaicleIgnorePath, "");
  }
  return dbsaicleIgnorePath;
}

export function getDbSaicleGlobalPath(): string {
  // This is ~/.dbsaicle on mac/linux
  const dbsaiclePath = DBSAICLE_GLOBAL_DIR;
  if (!fs.existsSync(dbsaiclePath)) {
    fs.mkdirSync(dbsaiclePath);
  }
  return dbsaiclePath;
}

export function getSessionsFolderPath(): string {
  const sessionsPath = path.join(getDbSaicleGlobalPath(), "sessions");
...
export function getIndexFolderPath(): string {
  const indexPath = path.join(getDbSaicleGlobalPath(), "index");
...
export function getSharedConfigFilePath(): string {
  return path.join(getDbSaicleGlobalPath(), "sharedConfig.json");
}
...
export function getConfigJsonPath(): string {
  const p = path.join(getDbSaicleGlobalPath(), "config.json");
  return p;
}

export function getConfigYamlPath(ideType?: IdeType): string {
  const p = path.join(getDbSaicleGlobalPath(), "config.yaml");
  if (!fs.existsSync(p) && !fs.existsSync(getConfigJsonPath())) {
    if (ideType === "jetbrains") {
      // https://github.com/dbsaicledev/continue/pull/7224
...
export function getConfigTsPath(): string {
  const p = path.join(getDbSaicleGlobalPath(), "config.ts");
  if (!fs.existsSync(p)) {
    fs.writeFileSync(p, DEFAULT_CONFIG_TS_CONTENTS);
  }

  const typesPath = path.join(getDbSaicleGlobalPath(), "types");
  ...
  const packageJsonPath = path.join(getDbSaicleGlobalPath(), "package.json");
  if (!fs.existsSync(packageJsonPath)) {
    fs.writeFileSync(
      packageJsonPath,
      JSON.stringify({
        name: "dbsaicle-config",
        version: "1.0.0",
        description: "My DbSaicle Configuration",
        main: "config.js",
      }),
    );
  }
...
export function getConfigJsPath(): string {
  ...
  return path.join(getDbSaicleGlobalPath(), "out", "config.js");
}

export function getTsConfigPath(): string {
  const tsConfigPath = path.join(getDbSaicleGlobalPath(), "tsconfig.json");
...
export function getDbSaicleRcPath(): string {
  // Disable indexing of the config folder to prevent infinite loops
  const dbsaiclercPath = path.join(getDbSaicleGlobalPath(), ".dbsaiclerc.json");
  if (!fs.existsSync(dbsaiclercPath)) {
    fs.writeFileSync(
      dbsaiclercPath,
      JSON.stringify(
        {
  ...
  return dbsaiclercPath;
}

function getDevDataPath(): string {
  const sPath = path.join(getDbSaicleGlobalPath(), "dev_data");
...
function editConfigJson(
  callback: (config: SerializedDbSaicleConfig) => SerializedDbSaicleConfig,
): void {
...
export function editConfigFile(
  configJsonCallback: (
    config: SerializedDbSaicleConfig,
  ) => SerializedDbSaicleConfig,
...
function getMigrationsFolderPath(): string {
  const migrationsPath = path.join(getDbSaicleGlobalPath(), ".migrations");
...
export function getRemoteConfigsFolderPath(): string {
  const dir = path.join(getDbSaicleGlobalPath(), ".configs");
...
export function getDbSaicleDotEnv(): { [key: string]: string } {
  const filepath = path.join(getDbSaicleGlobalPath(), ".env");
  if (fs.existsSync(filepath)) {
...
export function getLogsDirPath(): string {
  const logsPath = path.join(getDbSaicleGlobalPath(), "logs");
...
export function getGlobalFolderWithName(name: string): string {
  return path.join(getDbSaicleGlobalPath(), name);
}
...
export function getRepoMapFilePath(): string {
  return path.join(getDbSaicleUtilsPath(), "repo_map.txt");
}

export function getEsbuildBinaryPath(): string {
  return path.join(getDbSaicleUtilsPath(), "esbuild");
}
...
export function getLocalEnvironmentDotFilePath(): string {
  return path.join(getDbSaicleGlobalPath(), ".local");
}

export function getStagingEnvironmentDotFilePath(): string {
  return path.join(getDbSaicleGlobalPath(), ".staging");
}

export function getDiffsDirectoryPath(): string {
  const diffsPath = path.join(getDbSaicleGlobalPath(), ".diffs"); // .replace(/^C:/, "c:"); ??
