---
title: "DbSaicle Autocomplete Setup and Configuration Guide"
description: "Step-by-step guide to setting up and configuring autocomplete in DbSaicle, including Codestral, Ollama, and IDE settings."
...
## How to Set Up Autocomplete in DbSaicle with Codestral (Recommended)
...
<Tabs>
  <Tab title="Hub">
  [Mistral Codestral model block](https://hub.dbsaicle.dev/mistral/codestral)
  </Tab>
  <Tab title="YAML">
...
## How to Set Up Autocomplete in DbSaicle with Ollama (Local Model)
...
<Tabs>
  <Tab title="Hub">
  [Ollama Qwen 2.5 Coder 1.5B model block](https://hub.dbsaicle.dev/ollama/qwen2.5-coder-1.5b)
  </Tab>
  <Tab title="YAML">
  ```yaml title="config.yaml"
  models:
...
## Autocomplete Configuration Options in DbSaicle

### Autocomplete Models Available on the DbSaicle Hub

Explore autocomplete model configurations on [the hub](https://hub.dbsaicle.dev/explore/models?roles=autocomplete)

### Customize Autocomplete User Settings in the DbSaicle Extension
...
5. Check the output of the logs to find any potential errors: <kbd>cmd/ctrl</kbd> + <kbd>shift</kbd> + <kbd>P</kbd> -> "Toggle Developer Tools" -> "Console" tab in VS Code, ~/.dbsaicle/logs/core.log in JetBrains.
...
### How to Turn Off Autocomplete in DbSaicle (VS Code and JetBrains)

#### VS Code

Click the "DbSaicle" button in the status panel at the bottom right of the screen. The checkmark will become a "cancel" symbol and you will no longer see completions. You can click again to turn it back on.

Alternatively, open VS Code settings, search for "DbSaicle" and uncheck the box for "Enable Tab Autocomplete".
...
Open Settings -> Tools -> DbSaicle and uncheck the box for "Enable Tab Autocomplete".
