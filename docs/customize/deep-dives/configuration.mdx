---
title: "How to Configure DbSaicle Agents"
description: Learn how to access and manage DbSaicle agent configurations through Hub or local YAML files
keywords: [config, settings, customize]
sidebarTitle: "Agent Configuration"
---

You can easily access your agent configuration from the DbSaicle Chat sidebar. Open the sidebar by pressing <kbd>cmd/ctrl</kbd> + <kbd>L</kbd> (VS Code) or <kbd>cmd/ctrl</kbd> + <kbd>J</kbd> (JetBrains) and click the Agent selector above the main chat input. Then, you can hover over an agent and click the `new window` (hub agents) or `gear` (local agents) icon.

![configure an agent](/images/configure-dbsaicle.png)

## How to Manage Hub Agents

Hub Agents can be managed in [the Hub](https://hub.dbsaicle.dev). See [Editing an Agent](../../hub/agents/edit-an-agent.md)
...
- `~/.dbsaicle/config.yaml` (MacOS / Linux)
- `%USERPROFILE%\.dbsaicle\config.yaml` (Windows)
...
When editing this file, you can see the available options suggested as you type, or check the reference below. When you save a config file from the IDE, DbSaicle will automatically refresh to take into account your changes. A config file is automatically created the first time you use DbSaicle, and always automatically generated with default values if it doesn't exist.
...
- [`.dbsaiclerc.json`](#how-to-use-dbsaiclercjson-for-workspace-configuration) - Workspace-level configuration
- [`config.ts`](#how-to-use-configts-for-advanced-configuration) - Advanced configuration (probably unnecessary) - a TypeScript file in your home directory that can be used to programmatically modify (_merged_) the `config.json` schema:
  - `~/.dbsaicle/config.ts` (MacOS / Linux)
  - `%USERPROFILE%\.dbsaicle\config.ts` (Windows)

### How to Use `.dbsaiclerc.json` for Workspace Configuration

The format of `.dbsaiclerc.json` is the same as `config.json`, plus one _additional_ property `mergeBehavior`, which can be set to either "merge" or "overwrite". If set to "merge" (the default), `.dbsaiclerc.json` will be applied on top of `config.json` (arrays and objects are merged). If set to "overwrite", then every top-level property of `.dbsaiclerc.json` will overwrite that property from `config.json`.

Example

```json title=".dbsaiclerc.json"
{
...
`config.yaml` or `config.json` can handle the vast majority of necessary configuration, so we recommend using it whenever possible. However, if you need to programmatically extend DbSaicle configuration, you can use a `config.ts` file, placed at `~/.dbsaicle/config.ts` (MacOS / Linux) or `%USERPROFILE%\.dbsaicle\config.ts` (Windows).
