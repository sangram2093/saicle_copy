---
title: "How to Set Up Model Context Protocol (MCP) in DbSaicle"
...
[here](https://hub.dbsaicle.dev/explore/mcp).
...
```yaml title=".dbsaicle/mcpServers/playwright-mcp.yaml"
...
## How to Set Up DbSaicle Documentation Search with MCP

You can set up an MCP server to search the DbSaicle documentation directly from your agent. This is particularly useful for getting help with DbSaicle configuration and features.

For complete setup instructions, troubleshooting, and usage examples, see the [DbSaicle MCP Reference](/reference/dbsaicle-mcp).
...
`mcpServers`
block](https://hub.dbsaicle.dev/new?type=block&blockType=mcpServers) or add a local MCP
...
as well as access hosted secrets in the DbSaicle Hub. To leverage Hub secrets, you can use the `inputs` property in your MCP env block instead of `secrets`.
