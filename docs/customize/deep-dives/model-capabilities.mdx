---
title: "How to Configure Model Capabilities in DbSaicle"
...
DbSaicle needs to know what features your models support to provide the best experience. This guide explains how model capabilities work and how to configure them.

## What Are Model Capabilities?

Model capabilities tell DbSaicle what features a model supports:
...
## How DbSaicle Detects Model Capabilities

DbSaicle uses a two-tier system for determining model capabilities:

### How Automatic Detection Works (Default)

DbSaicle automatically detects capabilities based on your provider and model name. For example:
...
- [toolSupport.ts](https://github.com/dbsaicledev/continue/blob/main/core/llm/toolSupport.ts) - Tool capability detection logic
- [@dbsaicledev/llm-info](https://www.npmjs.com/package/@dbsaicledev/llm-info) - Image support detection

### How to Configure Capabilities Manually

You can add capabilities to models that DbSaicle doesn't automatically detect in your `config.yaml`.

<Note>
  You cannot override autodetection - you can only add capabilities. DbSaicle
...
2. **Using newer models** - DbSaicle doesn't yet recognize a newly released model
...
Add tool support for a model that DbSaicle doesn't recognize:
...
<Warning>
  An empty capabilities array does not disable autodetection. DbSaicle will
...
For troubleshooting capability-related issues like Agent mode being unavailable or tools not working, see the [Troubleshooting guide](/troubleshooting#agent-mode-is-unavailable-or-tools-arenâ€™t-working).
...
3. **Keep DbSaicle updated** - Newer versions improve autodetection

Remember: Setting capabilities only adds to autodetection. DbSaicle will still use its built-in knowledge about your model in addition to your specified capabilities.

## Model Capability Support

This matrix shows which models support tool use and image input capabilities. DbSaicle auto-detects these capabilities, but you can override them if needed.
