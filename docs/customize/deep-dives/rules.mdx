---
title: "How to Create and Manage Rules in DbSaicle"
description: "Rules are used to provide instructions to the model for Agent, Chat, and Edit requests."
keywords: [rules, .dbsaiclerules, system, prompt, message]
sidebarTitle: "Rules"
---
...
1. Create a folder called `.dbsaicle/rules` at the top level of your workspace
2. Add a file called `pirates-rule.md` to this folder.
3. Write the following contents to `pirates-rule.md` and save.

```md title=".dbsaicle/rules/pirates-rule.md"
---
name: Pirate rule
---
...
For example, you can say "Create a rule for this", and a rule will be created for you in `.dbsaicle/rules` based on your conversation.

</Info>

Rules can also be added to an Agent on the DbSaicle Hub.

Explore available rules [here](https://hub.dbsaicle.dev), or [create your own](https://hub.dbsaicle.dev/new?type=block&blockType=rules) in the Hub.
...
globs: docs/**/*.{md,mdx}
alwaysApply: false
description: Standards for writing and maintaining DbSaicle Docs
---

# DbSaicle Docs Standards
...
You can create project-specific rules by adding a `.dbsaicle/rules` folder to the root of your project and adding new rule files.
...
```md title=".dbsaicle/rules/typescript.md"
---
...
DbSaicle includes a simple default system message for [Agent](../../features/agent/quick-start) and [Chat](../../features/chat/quick-start) requests, to help the model provide reliable codeblock formats in its output.

This can be viewed in the rules section of the toolbar (see above), or in the source code [here](https://github.com/dbsaicledev/continue/blob/main/core/llm/constructMessages.ts#L4).
...
### Legacy: .dbsaiclerules File (Deprecated)

<Warning>
  `.dbsaiclerules` will be deprecated in a future release. Please use the
  `.dbsaicle/rules` folder instead.
</Warning>

You can create project-specific rules by adding a `.dbsaiclerules` file to the root of your project. This file is raw text and its full contents will be used as rules.
