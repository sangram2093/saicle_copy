...
description: "Configure OpenVINO Model Server with DbSaicle to use Intel-optimized models for CPU, iGPU, GPU and NPU via the OpenAI-compatible API, supporting code completion with models like CodeLlama and Qwen"
---

<Info>
  [**OpenVINOâ„¢ Mode Server**](https://github.com/openvinotoolkit/model_server)
  is scalable inference server for models optimized with OpenVINOâ„¢ for Intel
  CPU, iGPU, GPU and NPU.
</Info>

OpenVINOâ„¢ Mode Server supports text generation via OpenAI Chat Completions API. Simply select OpenAI provider to point `apiBase` to running OVMS instance. Refer [to this demo](https://docs.openvino.ai/2025/model-server/ovms_demos_code_completion_vsc.html) on official OVMS documentation to easily set up your own local server.
