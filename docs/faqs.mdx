---
title: "FAQs"
description: "Frequently asked questions about DbSaicle"
---
...
DbSaicle can be used in [code-server](https://coder.com/), but if you are running across an error in the logs that includes "This is likely because the editor is not running in a secure context", please see [their documentation on securely exposing code-server](https://coder.com/docs/code-server/latest/guide#expose-code-server).

## Changes to agents not showing in VS Code

If you've made changes to agents (adding, modifying, or removing them) but the changes aren't appearing in the DbSaicle extension in VS Code, try reloading the VS Code window:
...
## I installed DbSaicle, but don't see the sidebar window

By default the DbSaicle window is on the left side of VS Code, but it can be dragged to right side as well, which we recommend in our tutorial. In the situation where you have previously installed DbSaicle and moved it to the right side, it may still be there. You can reveal DbSaicle either by using cmd/ctrl+L or by clicking the button in the top right of VS Code to open the right sidebar.
...
If you have entered a valid API key and model, but are still getting a 404 error from OpenRouter, this may be because models that do not support function calling will return an error to DbSaicle when a request is sent. Example error: `HTTP 404 Not Found from https://openrouter.ai/api/v1/chat/completions`
...
This can be accomplished using the following command: `DbSaicle: Rebuild codebase index`.
...
<Info>
  DbSaicle uses system message tools as a fallback for models without native tool support, so most models should work with Agent mode automatically.
</Info>

### Check if your model has tool support

1. Not all models support native tool/function calling, but DbSaicle will automatically use system message tools as a fallback
...
If DbSaicle's autodetection isn't working correctly, you can manually add capabilities in your `config.yaml`:
...
To see what capabilities DbSaicle detected for your model:
...
This can be fixed by selecting `Actions > Choose Boot runtime for the IDE` then selecting the latest version, and then restarting Android Studio. [See this thread](https://github.com/dbsaicledev/continue/issues/596#issuecomment-1789327178) for details.
...
Most DbSaicle features will work normally, including autocomplete and chat. However, commands that rely on codebase indexing, such as `@codebase`, `@files`, and `@folder`, will be disabled.
...
When running DbSaicle or other tools in Docker that need to connect to Ollama on the host:
...
For running DbSaicle completely offline without internet access, see the [Running DbSaicle Without Internet guide](/guides/running-dbsaicle-without-internet).

DbSaicle supports multiple methods for managing secrets locally, searched in this order:

1. **Workspace `.env` files**: Place a `.env` file in your workspace root directory
2. **Workspace DbSaicle folder**: Place a `.env` file in `<workspace-root>/.dbsaicle/.env`  
3. **Global `.env` file**: Place a `.env` file in `~/.dbsaicle/.env` for user-wide secrets
4. **Process environment variables**: Use standard system environment variables

#### Creating `.env` files

Create a `.env` file in one of these locations:
- **Per-workspace**: `<workspace-root>/.env` or `<workspace-root>/.dbsaicle/.env`
- **Global**: `~/.dbsaicle/.env`
...
For centralized team secret management, use `${{ inputs.SECRET_NAME }}` syntax in your config.yaml and manage them at [https://hub.dbsaicle.dev/settings/secrets](https://hub.dbsaicle.dev/settings/secrets):
```yaml
models:
  - provider: openai
...
- Secrets are loaded when DbSaicle starts, so restart your IDE after changes
...
You can leverage model addons from the DbSaicle Hub in your local agent configurations using the `uses:` syntax. This allows you to reference pre-configured model blocks without duplicating configuration.

#### Requirements

- You must be logged in to DbSaicle
...
DbSaicle stores its data in the `~/.dbsaicle` directory (`%USERPROFILE%\.dbsaicle` on Windows).
