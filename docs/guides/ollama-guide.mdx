---
title: "Using Ollama with DbSaicle: A Developer's Guide"
description: "Complete guide to setting up Ollama with DbSaicle for local AI development. Learn installation, configuration, model selection, performance optimization, and troubleshooting for privacy-focused offline coding assistance"
---
...
## How to Configure Ollama with DbSaicle

There are multiple ways to configure Ollama models in DbSaicle:

### Method 1: Using Hub Model Blocks in Local config.yaml

The easiest way is to use [pre-configured model blocks](/reference#local-blocks) from the DbSaicle Hub in your local configuration:

```yaml title="~/.dbsaicle/agents/My Local Agent.yaml"
name: My Local Agent
version: 0.0.1
schema: v1
models:
...
<Warning>
  **Important**: Hub blocks only provide configuration - you still need to pull
  the model locally. The hub block `ollama/deepseek-r1-32b` configures DbSaicle
  to use `model: deepseek-r1:32b`, but the actual model must be installed:
  ```bash
  # Check what the hub block expects (view on hub.dbsaicle.dev)
...
DbSaicle can automatically detect available Ollama models. You can configure this in your YAML:

```yaml title="~/.dbsaicle/config.yaml"
models:
  - name: Autodetect
    provider: ollama
    model: AUTODETECT
...
3. DbSaicle will scan for available Ollama models
...
<Note>
  ...
  available models. When set to `AUTODETECT`, DbSaicle will dynamically populate
...
async def create_user(user: User):
    # DbSaicle will help complete this implementation
...
Ollama with DbSaicle provides a powerful local development environment for AI-assisted coding. You now have complete control over your AI models, ensuring privacy and enabling offline development workflows.

---

_This guide is based on Ollama v0.11.x and DbSaicle v1.1.x. Please check for updates regularly._
