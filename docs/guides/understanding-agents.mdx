...
description: "Learn how to choose between cloud-managed Hub agents and local configuration for AI development assistance in DbSaicle, including setup, management, and best practices for each approach"
---

Every developer has unique needs when it comes to AI assistance. Some prefer the convenience of cloud-managed configurations, while others need the control and privacy of local setups. DbSaicle offers both paths, and this guide will help you choose the right one for your workflow.

## What Are the Two Paths to AI Assistance?

DbSaicle provides two distinct ways to configure your AI agent:

Think of DbSaicle's configuration options like choosing between a managed service and self-hosting. Both get you to the same destinationâ€”powerful AI assistance in your IDEâ€”but the journey and control level differ significantly.
...
1. Open the DbSaicle Chat sidebar by pressing <kbd>cmd/ctrl</kbd> + <kbd>L</kbd> (VS Code) or <kbd>cmd/ctrl</kbd> + <kbd>J</kbd> (JetBrains)
...
![configure an agent](/images/configure-dbsaicle.png)

## What Are Hub Agents: The Managed Experience

Hub Agents represent the "it just works" philosophy. When you [sign in to DbSaicle Hub](https://auth.dbsaicle.dev/), you gain access to a curated ecosystem of pre-configured agents that sync seamlessly across all your development environments.

### Why Should You Choose Hub Agents?

**The Power of Simplicity**

- **Instant Setup**: Browse the [agent marketplace](https://hub.dbsaicle.dev) and add any agent to your account with a single click
...
1. **Select Your Agent**: Click the agent selector in your IDE's DbSaicle panel
2. **Explore or Create**: Browse community agents or craft your own specialized helper
3. **Secure Your Keys**: Add API keys as [User Secrets](https://hub.dbsaicle.dev/settings/secrets) in the hubâ€”they're encrypted and never exposed
...
All Hub Agent management happens through [the Hub](https://hub.dbsaicle.dev). For detailed customization, see our guide on [Editing an Agent](/hub/agents/edit-an-agent).
...
- macOS/Linux: `~/.dbsaicle/config.yaml`
- Windows: `%USERPROFILE%\.dbsaicle\config.yaml`
...
When you edit your `config.yaml`, DbSaicle provides intelligent autocomplete for all available options. Save the file, and DbSaicle automatically reloads your configurationâ€”no restart required.

The first time you use DbSaicle, it generates a `config.yaml` with sensible defaults. From there, you can customize everything from model selection to context providers, slash commands, and more.
...
- Check YAML syntax (DbSaicle will show errors)
- Ensure file permissions allow reading

**Autocomplete Not Working?**

- Update to the latest DbSaicle version
...
Happy coding with DbSaicle! ðŸš€
