...
description: "Learn how to create and remix reusable blocks in DbSaicle Hub for sharing configuration components like models, prompts, and tools with customizable inputs and template variables"
---
...
After filling out information on the block, you will want to create a block following the `config.yaml` [reference documentation](/reference). Refer to examples of blocks on [hub.dbsaicle.dev](https://hub.dbsaicle.dev/explore/models) and visit the [YAML Reference](/reference#complete-yaml-config-example) docs for more details.
