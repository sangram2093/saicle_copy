...
description: "Comprehensive guide to the config.yaml format used by DbSaicle.dev for building custom coding agents. Learn how to define models, context providers, rules, prompts, and more using reusable blocks and YAML configuration."
---

## Introduction

DbSaicle hub agents are defined using the `config.yaml` specification. Agents can be loaded from [the Hub](https://hub.dbsaicle.dev/explore/agents) or locally

- [DbSaicle Hub](https://hub.dbsaicle.dev/explore/agents) - YAML is stored on the hub and automatically synced to the extension

- Locally
  - in your global `.dbsaicle` folder (`~/.dbsaicle` on Mac, `%USERPROFILE%\.dbsaicle`) within `.dbsaicle/agents`. The name of the file will be used as the display name of the agent, e.g. `My Agent.yaml`
  - in your workspace in a `/.dbsaicle/agents` folder, with the same naming convention
...
Examples of blocks and agents can be found on the [DbSaicle Hub](https://hub.dbsaicle.dev).

Agents can either explicitly define blocks - see [Properties](#properties) below - or import and configure existing hub blocks.

### Using Blocks

Hub blocks and agents are identified with a slug in the format `owner-slug/block-or-agent-slug`, where an owner can be a user or organization (For example, if you want to use the [OpenAI 4o Model block](https://hub.dbsaicle.dev/openai/gpt-4o), your slug would be `openai/gpt-4o`). These blocks are pulled from [https://hub.dbsaicle.dev](https://hub.dbsaicle.dev).
...
It is also possible to define blocks locally in a `.dbsaicle` folder. This folder can be located at either the root of your workspace (these will automatically be applied to all agents when you are in that workspace) or in your home directory at `~/.dbsaicle` (these will automatically be applied globally).

Place your YAML files in the following folders:

Agents:

- `.dbsaicle/agents` - for agents

Blocks:

- `.dbsaicle/rules` - for rules
- `.dbsaicle/models` - for models
- `.dbsaicle/prompts` - for prompts
- `.dbsaicle/context` - for context providers
- `.dbsaicle/docs` - for docs
- `.dbsaicle/data` - for data
- `.dbsaicle/mcpServers` - for MCP Servers

You can find many examples of each of these block types on the [DbSaicle Explore Page](https://hub.dbsaicle.dev/explore/models)

<Info>
Local blocks utilizing mustache notation for secrets (`${{ secrets.SECRET_NAME }}`) can read secret values:

- globally, from a `.env` located in the global `.dbsaicle` folder (`~/.dbsaicle/.env`)
...
- `capabilities`: Array of strings denoting model capabilities, which will overwrite DbSaicle's autodetection based on provider and model. See the [Model Capabilities guide](/customize/deep-dives/model-capabilities) for detailed information. Supported capabilities include:
  ...
  DbSaicle automatically detects these capabilities for most models, but you can override this when using custom deployments or if autodetection isn't working correctly.
...
docs:
  - name: DbSaicle
    startUrl: https://docs.dbsaicle.dev/intro
    favicon: https://docs.dbsaicle.dev/favicon.ico
```
...
The [Model Context Protocol](https://modelcontextprotocol.io/introduction) is a standard proposed by Anthropic to unify prompts, context, and tool use. DbSaicle supports any MCP server with the MCP context provider.
...
data:
  - name: Local Data Bank
    destination: file:///Users/dallin/Documents/code/dbsaicledev/dbsaicle-extras/external-data
    schema: 0.2.0
    level: all
