...
<Warning>
  **This feature is deprecated.** The `@Codebase` context provider has been deprecated in favor of a more integrated approach to codebase awareness. Please refer to our [Guide on Making Agents Aware of Codebases and Documentation](/guides/codebase-documentation-awareness) for the recommended approach.
</Warning>
...
- Better integration with DbSaicle's Agent features
...
DbSaicle indexes your codebase so that it can later automatically pull in the most relevant context from throughout your workspace. This is done via a combination of embeddings-based retrieval and keyword search. By default, all embeddings are calculated locally using `transformers.js` and stored locally in `~/.dbsaicle/index`.
...
DbSaicle respects `.gitignore` files in order to determine which files should not be indexed. If you'd like to exclude additional files, you can add them to a `.dbsaicleignore` file, which follows the exact same rules as `.gitignore`.

DbSaicle also supports a **global** `.dbsaicleignore` file that will be respected for all workspaces, which can be created at `~/.dbsaicle/.dbsaicleignore`.

If you want to see exactly what files DbSaicle has indexed, the metadata is stored in `~/.dbsaicle/index/index.sqlite`. You can use a tool like [DB Browser for SQLite](https://sqlitebrowser.org/) to view the `tag_catalog` table within this file.
