...
description: "Comprehensive guide to resolving common issues with DbSaicle, including logging, keyboard shortcuts, networking problems, model capabilities, and extension configuration troubleshooting"
---
...
#### Prompt Logs (DbSaicle Console)

To view prompt logs/analytics, you can enable the DbSaicle Console.

1. Open VS Code settings (`cmd/ctrl` + `,`)
2. Search for the setting "DbSaicle: Enable Console" and enable it
3. Reload the window
4. Open the DbSaicle Console by using the command palette (`cmd/ctrl` + `shift` + `P`) and searching for "DbSaicle: Focus on DbSaicle Console View"

![DbSaicle Console](/images/images/dbsaicle-console-d387a10c2918c117c6c253a3b5f18c22.png)

### JetBrains

Open `~/.dbsaicle/logs/core.log` to view the logs for the DbSaicle plugin. The most recent logs are found at the bottom of the file.
...
We are constantly making fixes and improvements to DbSaicle, but the latest changes remain in a "pre-release" version for roughly a week so that we can test their stability. If you are experiencing issues, you can try the pre-release by going to the DbSaicle extension page in VS Code and selecting "Switch to Pre-Release" as shown below.
...
For VS Code, All versions are hosted on the Open VSX Registry [here](https://open-vsx.org/extension/DbSaicle/continue). Once you've downloaded the extension, which will be a .vsix file, you can install it manually by following the instructions [here](https://code.visualstudio.com/docs/editor/extension-gallery#_install-from-a-vsix).

You can find older versions of the JetBrains extension on their [marketplace](https://plugins.jetbrains.com/plugin/22707-dbsaicle), which will walk you through installing from disk.

## Keyboard shortcuts not resolving

If your keyboard shortcuts are not resolving, you may have other commands that are taking precedence over the DbSaicle shortcuts. You can see if this is the case, and change your shortcut mappings, in the configuration of your IDE.
