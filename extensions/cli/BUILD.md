...
The DbSaicle CLI uses esbuild to bundle the application along with local packages (`@dbsaicledev/config-yaml` and `@dbsaicledev/openai-adapters`) into a single distributable file. This ensures that users who install the CLI from npm don't need to worry about local file references.
...
- Include local packages (`@dbsaicledev/config-yaml`, `@dbsaicledev/openai-adapters`) directly in the bundle
...
2. Users install via `npm install -g @dbsaicledev/cli`
