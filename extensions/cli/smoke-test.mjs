...
runTest("Help command", () => {
  ...
  if (!output.includes("DbSaicle CLI") || !output.includes("--version")) {
...
runTest("Local packages are bundled", () => {
  ...
  // Check for code from @dbsaicledev/config-yaml
  if (
    !bundleContent.includes("AssistantUnrolled") &&
    !bundleContent.includes("config-yaml")
  ) {
    throw new Error("@dbsaicledev/config-yaml not properly bundled");
  }

  // Check for code from @dbsaicledev/openai-adapters
  ...
  if (
    ...
    throw new Error("@dbsaicledev/openai-adapters not properly bundled");
  }
});
