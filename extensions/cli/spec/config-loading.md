...
1. **Environment Variable**: `DBSAICLE_API_KEY` environment variable
2. **File-Based Auth**: `~/.dbsaicle/auth.json` file
...
1. **CLI `--config` Flag** (highest priority)
   ...
   - Assistant slug (`owner/package`): Fetches from DbSaicle platform
...
3. **Default Resolution** (if no flag and no saved URI)
   ...
   - **config.yaml**: The saved config file at `~/.dbsaicle/config.yaml`
   - **Unauthenticated**: Falls back to `dbsaicledev/default-agent`
...
- Default assistant (`dbsaicledev/default-agent`)
...
### Environment Variable Auth (`DBSAICLE_API_KEY`)
...
- **Result**: Falls back to `dbsaicledev/default-agent`
