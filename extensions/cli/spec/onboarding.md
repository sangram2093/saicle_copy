...
2. Otherwise, we will present the user with two options:
   ...
   - Log in with DbSaicle: log them in, which will automatically create their assistant and then we can load the first assistant from the first org
   - Enter your Anthropic API key: let them enter the key, and then either create a ~/.dbsaicle/config.yaml with the following contents OR update the existing config.yaml to add the model
...
3. If there are no assistants in that org, then look for a local ~/.dbsaicle/config.yaml
