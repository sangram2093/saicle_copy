# OTLP Metrics Specification for DbSaicle CLI

This document specifies the OpenTelemetry Protocol (OTLP) metrics that should be emitted by the DbSaicle CLI to provide comprehensive observability and usage monitoring. The metrics are designed to be compatible with Claude Code dashboards for easy migration.

## Overview

The DbSaicle CLI should emit metrics that provide insights into:
...
| ------------------------------- | --------------------------------------------------------- | ------------------------------------ |
| `DBSAICLE_CLI_ENABLE_TELEMETRY` | Enables telemetry collection (required)                   | `1`                                  |
...
| `app.version`       | Current DbSaicle CLI version                                  | `OTEL_METRICS_INCLUDE_VERSION` (default: false)     |
...
### ✅ `dbsaicle_cli_lines_of_code_count`
...
### ✅ `dbsaicle_cli_pull_request_count`
...
### ✅ `dbsaicle_cli_cost_usage`

**Type:** Counter  
**Unit:** `USD`  
**Description:** Cost of the DbSaicle CLI session
...
### ❌ `dbsaicle_cli_code_edit_tool_decision`
...
### ✅ `dbsaicle_cli_active_time_total`
...
**Event Name:** `dbsaicle_cli_user_prompt`
...
**Event Name:** `dbsaicle_cli_tool_result`
...
**Event Name:** `dbsaicle_cli_api_request`
...
These metrics are unique to DbSaicle CLI and provide additional insights without conflicting with compatibility:

### Authentication Metrics

#### ✅ `dbsaicle_cli_auth_attempts`
...
The core metrics (`session_count`, `lines_of_code_count`, `token_usage`, `cost_usage`, etc.) use identical naming and attribute structures to Claude Code, allowing for easy dashboard migration by simply changing the metric prefix from `claude_code_*` to `dbsaicle_cli_*`.
...
- `dbsaicle_cli_code_edit_tool_decision` - requires user confirmation UI
- `dbsaicle_cli_mcp_connections` - needs MCP service monitoring
- `dbsaicle_cli_startup_time` - needs startup time tracking
