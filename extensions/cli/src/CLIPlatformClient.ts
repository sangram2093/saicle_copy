...
import { DefaultApiInterface } from "@dbsaicledev/sdk/dist/api";
...
export class CLIPlatformClient implements PlatformClient {
  ...
  private findSecretInLocalEnvFiles(fqsn: FQSN): SecretResult | undefined {
    ...
    // Then check in priority order: ~/.dbsaicle/.env, <workspace>/.dbsaicle/.env, <workspace>/.env
    const workspaceDir = process.cwd();
    const envPaths = [
      path.join(env.dbsaicleHome, ".env"),
      path.join(workspaceDir, ".dbsaicle", ".env"),
