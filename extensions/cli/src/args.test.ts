...
describe("processPromptOrRule (loadRuleFromHub integration)", () => {
  ...
  describe("loadRuleFromHub", () => {
    it("should successfully load a rule from the DbSaicle Hub", async () => {
      ...
      const result = await processPromptOrRule("dbsaicledev/sentry-nextjs");

      expect(result).toBe(ruleContent);
      expect(mockFetch).toHaveBeenCalledWith(
        new URL(
          "v0/dbsaicledev/sentry-nextjs/latest/download",
          "https://api.dbsaicle.dev/",
        ),
      );
    });
    ...
    it("should handle HTTP errors when loading from hub", async () => {
      ...
      await expect(
        processPromptOrRule("dbsaicledev/nonexistent-rule"),
      ).rejects.toThrow(
        'Failed to load rule from hub "dbsaicledev/nonexistent-rule": HTTP 404: Not Found',
      );
    });
    ...
    it("should handle network errors when loading from hub", async () => {
      ...
      await expect(
        processPromptOrRule("dbsaicledev/sentry-nextjs"),
      ).rejects.toThrow(
        'Failed to load rule from hub "dbsaicledev/sentry-nextjs": Network error',
      );
    });
    ...
    it("should handle empty zip file", async () => {
      ...
      await expect(
        processPromptOrRule("dbsaicledev/empty-rule"),
      ).rejects.toThrow(
        'Failed to load rule from hub "dbsaicledev/empty-rule": No rule content found in downloaded zip file',
      );
    });
    ...
    it("should handle zip with only directories", async () => {
      ...
      await expect(
        processPromptOrRule("dbsaicledev/directory-only"),
      ).rejects.toThrow(
        'Failed to load rule from hub "dbsaicledev/directory-only": No rule content found in downloaded zip file',
      );
    });
    ...
    it("should prefer markdown files and ignore other file types", async () => {
      ...
      const result = await processPromptOrRule("dbsaicledev/mixed-files");
    ...
    it("should handle multiple markdown files and use the first one", async () => {
      ...
      const result = await processPromptOrRule("dbsaicledev/multiple-rules");
    ...
    it("should handle malformed zip files", async () => {
      ...
      await expect(
        processPromptOrRule("dbsaicledev/corrupted-zip"),
      ).rejects.toThrow(
        'Failed to load rule from hub "dbsaicledev/corrupted-zip"',
      );
    });
  });
