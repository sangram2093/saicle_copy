...
// Config file path
const AUTH_CONFIG_PATH = path.join(env.dbsaicleHome, "auth.json");
...
export function loadAuthConfig(): AuthConfig {
  // If DBSAICLE_API_KEY environment variable exists, use that instead
  if (process.env.DBSAICLE_API_KEY) {
    return {
      accessToken: process.env.DBSAICLE_API_KEY,
      organizationId: null,
    };
  }
...
export function saveAuthConfig(config: AuthenticatedConfig): void {
  // If using DBSAICLE_API_KEY environment variable, don't save anything
  if (process.env.DBSAICLE_API_KEY) {
    return;
  }
...
export async function login(): Promise<AuthConfig> {
  ...
  if (process.env.DBSAICLE_API_KEY) {
    console.info(
      chalk.green("Using DBSAICLE_API_KEY from environment variables"),
    );
    return {
      accessToken: process.env.DBSAICLE_API_KEY,
      organizationId: null,
...
> {
  ...
  // If using DBSAICLE_API_KEY environment variable, organization switching is not supported
  if (isEnvironmentAuthConfig(authConfig)) {
    return null;
  }
...
export function logout(): void {
  ...
  if (process.env.DBSAICLE_API_KEY) {
    console.info(
      chalk.yellow(
        "Using DBSAICLE_API_KEY from environment variables, nothing to log out",
      ),
    );
    return;
  }
