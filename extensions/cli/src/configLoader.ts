...
function determineConfigSource(
  ...
  if (authConfig === null) {
    ...
    const defaultConfigPath = path.join(env.dbsaicleHome, "config.yaml");
...
/**
 * Loads default config.yaml from ~/.dbsaicle/config.yaml
 */
...
/**
 * Loads the default dbsaicledev/default-agent
 */
...
/**
 * Loads an assistant by slug from the DbSaicle platform
 */
...
function getUriFromSource(source: ConfigSource): string | null {
  switch (source.type) {
    ...
    case "saved-uri":
      return source.uri;
    case "default-config-yaml":
      return `file://${path.join(env.dbsaicleHome, "config.yaml")}`;
    default:
      return null;
  }
}
