...
describe("loadMcpFromHub", () => {
  ...
  it("should handle HTTP errors", async () => {
    ...
    await expect(loadMcpFromHub("dbsaicledev/nonexistent-mcp")).rejects.toThrow(
      'Failed to load mcp from hub "dbsaicledev/nonexistent-mcp": HTTP 404: Not Found',
    );
  });
  ...
  it("should handle network errors", async () => {
    ...
    await expect(loadMcpFromHub("dbsaicledev/test-mcp")).rejects.toThrow(
      'Failed to load mcp from hub "dbsaicledev/test-mcp": Network error',
    );
  });
  ...
  it("should parse JSON configuration files", async () => {
    ...
    const result = await loadMcpFromHub("dbsaicledev/test-mcp");
  ...
  it("should handle missing configuration files", async () => {
    ...
    const result = await loadMcpFromHub("dbsaicledev/no-config");
  ...
  it("should handle invalid JSON configuration", async () => {
    ...
    await expect(loadMcpFromHub("dbsaicledev/invalid-json")).rejects.toThrow(
      'Failed to load mcp from hub "dbsaicledev/invalid-json": Invalid JSON',
    );
  });
});
