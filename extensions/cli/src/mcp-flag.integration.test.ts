...
describe("--mcp flag integration", () => {
  ...
  it("should demonstrate the complete --mcp flag functionality", () => {
    ...
    expect(args).toEqual({
      ...
      mcps: ["dbsaicledev/filesystem", "dbsaicledev/github"],
      prompt: "hello world",
    });

    // Verify MCP slugs are properly captured
    expect(args.mcps).toHaveLength(2);
    expect(args.mcps).toContain("dbsaicledev/filesystem");
    expect(args.mcps).toContain("dbsaicledev/github");
  });
