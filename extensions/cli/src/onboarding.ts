...
import { BaseLlmApi } from "@dbsaicledev/openai-adapters";
import { DefaultApiInterface } from "@dbsaicledev/sdk/dist/api/dist/index.js";
...
const CONFIG_PATH = path.join(env.dbsaicleHome, "config.yaml");
...
async function runOnboardingFlow(
  ...
  console.log(chalk.white("1. ⏩ Log in with DbSaicle"));
...
export async function runNormalFlow(
  ...
  // Step 3: Look for local ~/.dbsaicle/config.yaml
...
export async function isFirstTime(): Promise<boolean> {
  return !fs.existsSync(path.join(env.dbsaicleHome, ".onboarding_complete"));
}

export async function markOnboardingComplete(): Promise<void> {
  const flagPath = path.join(env.dbsaicleHome, ".onboarding_complete");
