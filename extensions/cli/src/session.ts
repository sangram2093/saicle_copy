...
function getSessionDir(): string {
  ...
  if (process.env.DBSAICLE_CLI_TEST && process.env.HOME) {
    const sessionDir = path.join(process.env.HOME, ".dbsaicle", "sessions");
  ...
  // Use DBSAICLE_GLOBAL_DIR if set (for testing)
  const dbsaicleHome =
    process.env.DBSAICLE_GLOBAL_DIR || path.join(os.homedir(), ".dbsaicle");
  const sessionDir = path.join(dbsaicleHome, "sessions");
...
class SessionManager {
  ...
  getCurrentSession(): Session {
    if (!this.currentSession) {
      ...
      const sessionId = process.env.DBSAICLE_CLI_TEST_SESSION_ID
        ? process.env.DBSAICLE_CLI_TEST_SESSION_ID
        : uuidv4();
