import type { AssistantUnrolled } from "@dbsaicledev/config-yaml";
import {
  describe,
  it,
  expect,
...
vi.mock("./env.js", () => ({
  env: {
    dbsaicleHome: "/home/test/.dbsaicle",
    appUrl: "https://hub.dbsaicle.dev",
  },
}));
...
vi.mock("./session.js", () => ({
  getSessionFilePath: vi.fn(
    () => "/home/test/.dbsaicle/cli-sessions/dbsaicle-cli-pid-12345.json",
  ),
  ...
  describe("handleSlashCommands", () => {
    ...
    it("should use test session directory when in test mode", async () => {
      ...
      (getSessionFilePath as any).mockReturnValue(
        "/test-home/.dbsaicle/cli-sessions/dbsaicle-cli-test-123.json",
      );
      ...
      expect(result?.output).toContain("/test-home/.dbsaicle/cli-sessions/");
