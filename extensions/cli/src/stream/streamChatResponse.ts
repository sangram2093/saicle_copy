...
function processChunk(options: ProcessChunkOptions): {
  ...
  if (!chunk.choices || !chunk.choices[0]) {
    return { aiResponse, shouldDbSaicle: true };
  }
  ...
  return { aiResponse: updatedResponse, shouldDbSaicle: true };
}
...
export async function processStreamingResponse(
  ...
  try {
    ...
    for await (const chunk of streamWithBackoff) {
      ...
      if (!result.shouldDbSaicle) break;
    }
  ...
  return {
    ...
    shouldDbSaicle: validToolCalls.length > 0,
  };
}
...
export async function streamChatResponse(
  ...
  while (true) {
    ...
    const { content, toolCalls, shouldDbSaicle } =
