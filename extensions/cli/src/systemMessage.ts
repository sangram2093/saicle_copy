...
function getDirectoryStructure(): string {
  try {
    const walker = new WalkerSync({
      ...
      includeEmpty: false,
      follow: false,
      ignoreFiles: [".gitignore", ".dbsaicleignore", ".customignore"],
    });
...
const baseSystemMessage = `You are an agent in the DbSaicle CLI. Given the user's prompt, you should use the tools available to you to answer the user's question.
...
Generated with [DbSaicle](https://dbsaicle.dev)

Co-Authored-By: DbSaicle <noreply@dbsaicle.dev>
</context>\n`;
  }
