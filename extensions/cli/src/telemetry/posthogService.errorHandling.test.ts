...
describe("PosthogService Error Handling", () => {
  ...
  it("should not capture when service is disabled", async () => {
    ...
    const originalEnv = process.env.DBSAICLE_CLI_ENABLE_TELEMETRY;
    process.env.DBSAICLE_CLI_ENABLE_TELEMETRY = "0";
    ...
    if (originalEnv === undefined) {
      delete process.env.DBSAICLE_CLI_ENABLE_TELEMETRY;
    } else {
      process.env.DBSAICLE_CLI_ENABLE_TELEMETRY = originalEnv;
    }
  });
});
