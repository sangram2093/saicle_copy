...
import { isDbSaicleRemoteAgent, isGitHubActions } from "../util/git.js";
...
class TelemetryService {
  ...
  private loadConfig(): TelemetryConfig {
    ...
    const enabled =
      process.env.DBSAICLE_CLI_ENABLE_TELEMETRY !== "0" && hasOtelConfig;
  ...
  private initialize() {
    try {
      ...
      const resource = resourceFromAttributes({
        [SEMRESATTRS_SERVICE_NAME]: "dbsaicle-cli",
      ...
      this.meter = metrics.getMeter("dbsaicle-cli", getVersion());
  ...
  private initializeMetrics() {
    ...
    this.commitCounter = this.meter.createCounter("dbsaicle_cli_commit_count", {
      description: "Number of git commits created",
      unit: "count",
    });

    this.costCounter = this.meter.createCounter("dbsaicle_cli_cost_usage", {
      description: "Cost of the DbSaicle CLI session",
      unit: "USD",
    });

    this.tokenCounter = this.meter.createCounter("dbsaicle_cli_token_usage", {
    ...
    // Additional DbSaicle CLI specific metrics
    ...
    this.slashCommandCounter = this.meter.createCounter(
      "dbsaicle_cli_slash_command_usage",
      {
  ...
  public recordSessionStart() {
    ...
    const sessionAttributes = this.getStandardAttributes({
      ...
      is_dbsaicle_remote_agent: isDbSaicleRemoteAgent().toString(),
    });
