...
describe("TUIChat - Tool Permission Tests", () => {
  ...
  it("shows ToolPermissionSelector with correct content", async () => {
    ...
    expect(frame).toContain("DbSaicle");
    expect(frame).toContain("DbSaicle + don't ask again");
    expect(frame).toContain("No, and tell DbSaicle what to do differently");
  ...
  it("handles arrow key navigation in permission selector", async () => {
    ...
    // Initial state should show "> DbSaicle" selected
    const frame = lastFrame();
    expect(frame).toMatch(/>\s+DbSaicle/);
  ...
  it("shows the new 'No, and tell DbSaicle what to do differently' option in the UI", async () => {
    const handleResponse = vi.fn();
  ...
  it("triggers stop stream when escape key is pressed", async () => {
    ...
    // Press escape key to trigger the "No, and tell DbSaicle what to do differently" option
    stdin.write("\x1b"); // ESC key
  ...
  it("triggers stop stream when 'n' key is pressed", async () => {
    ...
    // Press 'n' key to trigger the "No, and tell DbSaicle what to do differently" option
    stdin.write("n");
