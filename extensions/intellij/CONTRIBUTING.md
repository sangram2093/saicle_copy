# Contributing to DbSaicle (JetBrains extension) <!-- omit in toc -->
...
- [Development Workflow](#development-workflow)
  ...
  - [Accessing files in the `.dbsaicle` directory](#accessing-files-in-the-dbsaicle-directory)
...
DbSaicle is built with JDK version 17 (as specified in [`./build.gradle.kts`](./build.gradle.kts)), which can be
...
- [File Expander](https://plugins.jetbrains.com/plugin/11940-file-expander) - allows you to easily preview archives as
  directories (like `build/distributions/dbsaicle-*.zip`)
...
Select the `Run DbSaicle` task in the top right corner of the IDE and then select the "Debug" option.

> In community edition, use `Run DbSaicle (CE)` instead, which uses shell scripts instead of Ultimate-only node configs.
...
![run-extension-screenshot](../../media/run-dbsaicle-intellij.png)

This should open a new instance on IntelliJ with the extension installed.

### Accessing files in the `.dbsaicle` directory

When running the `Start Core Dev Server` task, we set the location of your DbSaicle directory to
`./extensions/.dbsaicle-debug`. This is to
allow for changes to your `config.json` and other files during development, without affecting your actual configuration.

### Viewing logs

When using the `Run DbSaicle` task, we automatically tail both prompt logs and IDE logs.
...
dependencies - Displays all dependencies declared in root project 'dbsaicle-intellij-extension'
...
```sh
./gradlew test --tests "com.github.dbsaicledev.dbsaicleintellijextension.e2e.TESTNAME"
```
...
`build/idea-sandbox/plugins-uiTest/dbsaicle-intellij-extension`
