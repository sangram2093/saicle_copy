package com.github.dbsaicledev.dbsaicleintellijextension.auth
...
@Service
class DbSaicleAuthService {
    private val coroutineScope = CoroutineScope(Dispatchers.IO)
    private val log = Logger.getInstance(DbSaicleAuthService::class.java)

    companion object {
        private const val CREDENTIALS_USER = "DbSaicleAuthUser"
        private const val ACCESS_TOKEN_KEY = "DbSaicleAccessToken"
        private const val REFRESH_TOKEN_KEY = "DbSaicleRefreshToken"
        private const val ACCOUNT_ID_KEY = "DbSaicleAccountId"
        private const val ACCOUNT_LABEL_KEY = "DbSaicleAccountLabel"
    }

    private fun getControlPlaneUrl(): String {
        val env = service<DbSaicleExtensionSettings>().dbsaicleState.dbsaicleTestEnvironment;
        when (env) {
            "none" -> return "https://control-plane-api-service-i3dqylpbqa-uc.a.run.app"
            "local" -> return "http://localhost:3001"
            "production" -> return "https://api.dbsaicle.dev"
            "test" -> return "https://api-test.dbsaicle.dev"
        }
    ...
    fun startAuthFlow(project: Project, useOnboarding: Boolean) {
        ...
        ApplicationManager.getApplication().invokeLater {
            val dialog = DbSaicleAuthDialog(useOnboarding, url) { token ->
                // Store the token
    ...
    private fun updateRefreshToken(token: String) {
        try {
            ...
            service<DbSaicleSentryService>().report(e, "Exception while refreshing token ${e.message}")
        }
    }
