...
class DbSaicleBrowser(private val project: Project): Disposable {

    private val log = Logger.getInstance(DbSaicleBrowser::class.java.simpleName)
    ...
    init {
        CefApp.getInstance().registerSchemeHandlerFactory("http", "dbsaicle", CustomSchemeHandlerFactory())
        ...
        myJSQueryOpenInBrowser.addHandler { msg: String? ->
            ...
            if (MessageTypes.PASS_THROUGH_TO_CORE.contains(messageType)) {
                project.service<DbSaiclePluginService>().coreMessenger?.request(messageType, data, messageId) { data ->
            ...
            if (msg != null) {
                project.service<DbSaiclePluginService>().ideProtocolClient?.handleMessage(msg) { data ->
                    sendToWebview(
                        messageType,
                        mapOf(
                            "status" to "success",
        ...
        project.service<DbSaiclePluginService>().onProtocolClientInitialized {
    ...
    private companion object {
        ...
        private fun getGuiUrl() =
            System.getenv("GUI_URL") ?: "http://dbsaicle/index.html"
