 ...
 */
package com.github.dbsaicledev.dbsaicleintellijextension.constants
...
fun getDbSaicleGlobalPath(): String {
  val dbsaiclePath = Paths.get(System.getProperty("user.home"), ".dbsaicle")
  if (Files.notExists(dbsaiclePath)) {
    Files.createDirectories(dbsaiclePath)
  }
  return dbsaiclePath.toString()
}

fun getDbSaicleRemoteConfigPath(remoteHostname: String): String {
  val path = Paths.get(getDbSaicleGlobalPath(), ".configs")
...
fun getConfigJsonPath(remoteHostname: String? = null): String {
  val path =
      Paths.get(
          if (remoteHostname != null) getDbSaicleRemoteConfigPath(remoteHostname)
          else getDbSaicleGlobalPath(),
          "config.json")
  if (Files.notExists(path)) {
...
fun getSessionsDir(): String {
  val path = Paths.get(getDbSaicleGlobalPath(), "sessions")
  if (Files.notExists(path)) {
    Files.createDirectories(path)
  }
  return path.toString()
