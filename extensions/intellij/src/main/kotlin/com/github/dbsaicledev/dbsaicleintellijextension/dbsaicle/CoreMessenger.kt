...
import com.github.dbsaicledev.dbsaicleintellijextension.constants.MessageTypes
import com.github.dbsaicledev.dbsaicleintellijextension.dbsaicle.process.DbSaicleBinaryProcess
import com.github.dbsaicledev.dbsaicleintellijextension.dbsaicle.process.DbSaicleProcessHandler
import com.github.dbsaicledev.dbsaicleintellijextension.dbsaicle.process.DbSaicleSocketProcess
...
class CoreMessenger(
    ...
    private var process = startDbSaicleProcess()
    ...
    private fun startDbSaicleProcess(): DbSaicleProcessHandler {
        val isTcp = System.getenv("USE_TCP")?.toBoolean() ?: false
        val process = if (isTcp)
            DbSaicleSocketProcess()
        else
            DbSaicleBinaryProcess(onUnexpectedExit)
        return DbSaicleProcessHandler(coroutineScope, process, ::handleMessage)
    }
    ...
    fun restart() {
        log.warn("Restarting DbSaicle process")
        responseListeners.clear()
        process.close()
        process = startDbSaicleProcess()
    }

    fun close() {
        log.warn("Closing DbSaicle process")
        process.close()
    }
}