...
import com.github.dbsaicledev.dbsaicleintellijextension.utils.getAltKeyLabel
...
fun getDiffDirectory(): File {
    ...
    val diffDirPath = Paths.get(homeDirectory).resolve(".dbsaicle").resolve(".diffs").toString()
...
class DiffManager(private val project: Project) : DumbAware {
    ...
    fun acceptDiff(file2: String?) {
        ...
        project.service<DbSaiclePluginService>().ideProtocolClient?.sendAcceptRejectDiff(true, diffInfo.stepIndex)
    ...
    fun rejectDiff(file2: String?) {
        ...
        dbsaiclePluginService.ideProtocolClient?.deleteAtIndex(diffInfo.stepIndex)
        dbsaiclePluginService.ideProtocolClient?.sendAcceptRejectDiff(false, diffInfo.stepIndex)
        dbsaiclePluginService.coreMessenger?.request("cancelApply", null, null) {}
        cleanUpFile(file)
    }
    ...
    private fun openDiffWindow(
        ...
        val diffRequest = SimpleDiffRequest("DbSaicle Diff", content1, content2, "Old", "New")
                ...
                val dialog: DialogWrapper = diffInfo?.dialog
                    ?: object : DialogWrapper(project, true, IdeModalityType.MODELESS) {
                        ...
                        override fun createActions(): Array<Action> {
                            ...
                            okAction.putValue(Action.NAME, "Accept (${getAltKeyLabel()} â‡§ Y)")

                            val cancelAction = cancelAction
                            cancelAction.putValue(Action.NAME, "Reject (${getAltKeyLabel()} â‡§ N)")
