package com.github.dbsaicledev.dbsaicleintellijextension.nextEdit
...
class DbSaicleNextEditService(private val project: Project) : NextEditService {
    private val displayedCompletions = mutableMapOf<String, NextEditOutcome>()

    private val coreMessenger: CoreMessenger?
        get() = project.service<DbSaiclePluginService>().coreMessenger
...
//                val nextEditItem = processedItem?.let { convertToNextEditItem(it) }
    ...
    private fun parseProcessedItem(response: Any?): ProcessedItem? {
        return try {
            ...
            val range = com.github.dbsaicledev.dbsaicleintellijextension.Range(startPosition, endPosition)
            val location = com.github.dbsaicledev.dbsaicleintellijextension.RangeInFile(filepath, range)
