...
export class GUIActions {
  public static moveDbSaicleToSidebar = async (driver: WebDriver) => {
    ...
    await GUIActions.executeFocusDbSaicleInputShortcut(driver);
  ...
  public static switchToReactIframe = async () => {
    ...
    let dbsaicleIFrame: WebElement | undefined = undefined;
    for (let i = 0; i < iframes.length; i++) {
      const iframe = iframes[i];
      const src = await iframe.getAttribute("src");
      if (src.includes("extensionId=DbSaicle.dbsaicle")) {
        dbsaicleIFrame = iframe;
        break;
      }
    }

    if (!dbsaicleIFrame) {
      throw new Error("Could not find DbSaicle iframe");
    }

    await driver.switchTo().frame(dbsaicleIFrame);
  ...
  public static toggleGui = async () => {
    return TestUtils.waitForSuccess(() =>
      new Workbench().executeCommand("dbsaicle.focusDbSaicleInput"),
    );
  };
  ...
  public static async executeFocusDbSaicleInputShortcut(driver: WebDriver) {
    return driver
      .actions()
