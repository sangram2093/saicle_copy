...
export class GlobalActions {
  static defaultFolder = "e2e/test-dbsaicle";
  ...
  static async setNextEditEnabled(enabled: boolean) {
    ...
    const dbsaicleItem = await TestUtils.waitForSuccess(async () => {
      ...
      const element = await statusBar.findElement(
        By.xpath("//*[contains(text(), 'DbSaicle')]"),
      );
    ...
    const text = await TestUtils.waitForSuccess(async () => {
      const itemText = await dbsaicleItem.getText();
      if (!itemText || itemText.trim() === "") {
        // Fallback to textContent
        const textContent = await dbsaicleItem.getAttribute("textContent");
        if (textContent && textContent.trim() !== "") {
          return textContent;
        }
    ...
    if (hasNE !== enabled) {
      await workbench.executeCommand("DbSaicle: Toggle Next Edit");
