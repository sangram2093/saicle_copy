...
async function downloadFile(url, destPath) {
  ...
  try {
    ...
    const command = `curl -L ${additionalCurlOptions} -o "${destPath}" "${url}"`;
    console.log(`[info] Downloading file from ${url} to ${destPath}`);
    console.log(`The command ${command} is being used.`);
    execSync(command, {
      stdio: "inherit", // Inherit stdio to show curl output
    });
  } catch (error) {
...
async function installAndCopyEsbuild(target) {
  ...
  execCmdSync("npm install esbuild");
  fs.cpSync(
    path.join("node_modules", "esbuild", "bin", "esbuild"),
    path.join("out", "node_modules", "@esbuild", `${target}`, "bin", "esbuild"),
  );
}
