...
const dbsaicleDir = path.join(__dirname, "..", "..", "..");

function generateConfigYamlSchema() {
  process.chdir(path.join(dbsaicleDir, "packages", "config-yaml"));
  execCmdSync("npm install");
  execCmdSync("npm run build");
  execCmdSync("npm run generate-schema");
  fs.copyFileSync(
    path.join("schema", "config-yaml-schema.json"),
    path.join(dbsaicleDir, "extensions", "vscode", "config-yaml-schema.json"),
  );
  console.log("[info] Generated config.yaml schema");
}

function copyConfigSchema() {
  process.chdir(path.join(dbsaicleDir, "extensions", "vscode"));
  // Modify and copy for .dbsaiclerc.json
  const schema = JSON.parse(fs.readFileSync("config_schema.json", "utf8"));
  schema.$defs.SerializedDbSaicleConfig.properties.mergeBehavior = {
    type: "string",
    enum: ["merge", "overwrite"],
    default: "merge",
    title: "Merge behavior",
    markdownDescription:
      "If set to 'merge', .dbsaiclerc.json will be applied on top of config.json (arrays and objects are merged). If set to 'overwrite', then every top-level property of .dbsaiclerc.json will overwrite that property from config.json.",
    "x-intellij-html-description":
      "<p>If set to <code>merge</code>, <code>.dbsaiclerc.json</code> will be applied on top of <code>config.json</code> (arrays and objects are merged). If set to <code>overwrite</code>, then every top-level property of <code>.dbsaiclerc.json</code> will overwrite that property from <code>config.json</code>.</p>",
  };
  fs.writeFileSync("dbsaicle_rc_schema.json", JSON.stringify(schema, null, 2));
...
function installNodeModules() {
  ...
  process.chdir(path.join(dbsaicleDir, "gui"));
...
//     execCmdSync(
//       `curl -o node_modules/@esbuild/esbuild.zip https://dbsaicle-server-binaries.s3.us-west-1.amazonaws.com/${target}/esbuild.zip`,
//     );
...
async function downloadEsbuildBinary(target) {
  ...
  execCmdSync(
    `curl -L ${additionalCurlOptions} -o out/tmp/esbuild.tgz ${downloadUrl}`,
  );
...
async function downloadSqliteBinary(target) {
  ...
  // Check for CURL_OPTIONS environment variable
  const additionalCurlOptions = process.env.CURL_OPTIONS
    ? process.env.CURL_OPTIONS
    : "";
  execCmdSync(
    `curl -L ${additionalCurlOptions} -o ../../core/node_modules/sqlite3/build.tar.gz ${downloadUrl}`,
  );
...
async function downloadRipgrepBinary(target) {
  ...
  if (target.startsWith("win")) {
    execCmdSync(
      `curl -L ${additionalCurlOptions}  -o node_modules/@vscode/ripgrep/bin/build.zip ${downloadUrl}`,
    );
    execCmdSync("cd node_modules/@vscode/ripgrep/bin && unzip build.zip");
    fs.unlinkSync("node_modules/@vscode/ripgrep/bin/build.zip");
  } else {
    execCmdSync(
      `curl -L ${additionalCurlOptions} -o node_modules/@vscode/ripgrep/bin/build.tar.gz ${downloadUrl}`,
    );
    execCmdSync(
...
async function installNodeModuleInTempDirAndCopyToCurrent(packageName, toCopy) {
  ...
  const tempDir = `/tmp/dbsaicle-node_modules-${adjustedName}`;
...
function writeBuildTimestamp() {
  fs.writeFileSync(
    path.join(dbsaicleDir, "extensions/vscode", "src/.buildTimestamp.ts"),
