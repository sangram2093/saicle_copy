...
class VsCodeIde implements IDE {
  ...
  setBrowserIntegration(browserIntegration: BrowserIntegration): void {
  ...
  async browserAction(
    ...
    if (!this.browserIntegration) {
      throw new Error("Browser integration is not available.");
    }
    return await this.browserIntegration.handleAction(request);
  }
  ...
  async isTelemetryEnabled(): Promise<boolean> {
    ...
    const dbsaicleEnabled: boolean =
      (await vscode.workspace
        .getConfiguration(EXTENSION_NAME)
        .get("telemetryEnabled")) ?? true;
    return globalEnabled && dbsaicleEnabled;
  }
