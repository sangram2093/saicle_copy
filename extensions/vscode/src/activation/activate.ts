import { getDbSaicleRcPath, getTsConfigPath } from "core/util/paths";
...
import { VsCodeDbSaicleApi } from "./api";
...
export async function activateExtension(context: vscode.ExtensionContext) {
  ...
  if (platformCheck.isUnsupported && !hasShownUnsupportedPlatformWarning) {
    ...
    void vscode.window.showInformationMessage(
      `DbSaicle detected that you are using ${platformTarget}. Due to native dependencies, DbSaicle may not be able to start`,
    );
  ...
  const yamlMatcher = ".dbsaicle/**/*.yaml";
  ...
  try {
    ...
    console.error(
      "Failed to register DbSaicle config.yaml schema, most likely, YAML extension is not installed",
      error,
    );
  }

  const api = new VsCodeDbSaicleApi(vscodeExtension);
  const dbsaiclePublicApi = {
