...
export class ApplyManager {
  ...
  async applyToFile({
    ...
    // if inbound text is a single JSON-style quoted string representing escaped
    // multiline code (e.g. "function foo(){\\n return 1; \\n}"), decode it so we
    // operate on real code instead of the literal quoted line
    let applyText = text;

    if (
      typeof applyText === "string" &&
      applyText.length > 1 &&
      applyText.startsWith('"') &&
      applyText.endsWith('"')
    ) {
      const inner = applyText.slice(1, -1);
      // Heuristic: presence of escaped newline/tgab/quote/backslash indicates encoded content
      const containsEscapes = /\\n|\\r|\\t|\\"|\\\\/.test(inner);
      const containsRealNewLine = /\n/.test(inner);
      if (containsEscapes && !containsRealNewLine) {
        try {
          // use JSON.parse for correct unescaping of all sequences.
          applyText = JSON.parse(inner);
        } catch (e) {
          // fallback to manual unscaping of all the sequences if JSON.parse fails (malformed escaping)
          applyText = inner
            .replace(/\\n/g, "\n")
            .replace(/\\r/g, "\r")
            .replace(/\\t/g, "\t")
    ...
    if (hasExistingDocument) {
      ...
      if (isSearchAndReplace) {
        const diffLinesGenerator = generateLines(
          myersDiff(activeTextEditor.document.getText(), applyText),
        );
