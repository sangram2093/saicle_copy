      ...
      .filter(
        (s) =>
          ...
          (s.filepath !== currentFilepath &&
            // Exclude DbSaicle's own output as it makes it super-hard for users to test the autocomplete feature
            // while looking at the prompts in the DbSaicle's output
            !s.filepath.startsWith(
              "output:extension-output-DbSaicle.dbsaicle",
            )),
      )
      .sort((a, b) => b.timestamp - a.timestamp)
