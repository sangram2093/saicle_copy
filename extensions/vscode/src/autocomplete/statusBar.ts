...
const statusBarItemText = (
  ...
  if (error) {
    return "$(alert) DbSaicle (config error)";
  }
  ...
  switch (status) {
    case undefined:
      if (loading) {
        text = "$(loading~spin) DbSaicle";
      } else {
        text = "DbSaicle";
      }
      break;
    case StatusBarStatus.Disabled:
      text = "$(circle-slash) DbSaicle";
      break;
    case StatusBarStatus.Enabled:
      text = "$(check) DbSaicle";
      break;
    case StatusBarStatus.Paused:
      text = "$(debug-pause) DbSaicle";
      break;
    default:
      text = "DbSaicle";
  }
...
export function setupStatusBar(
  ...
  statusBarItem.command = "dbsaicle.openTabAutocompleteConfigMenu";
  ...
  vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration(DBSAICLE_WORKSPACE_KEY)) {
      const enabled = getDbSaicleWorkspaceConfig().get<boolean>(
