...
import { DbSaicleConsoleWebviewViewProvider } from "./DbSaicleConsoleWebviewViewProvider";
import { DbSaicleGUIWebviewViewProvider } from "./DbSaicleGUIWebviewViewProvider";
...
function focusGUI() {
  // focus sidebar
  vscode.commands.executeCommand("dbsaicle.dbsaicleGUIView.focus");
...
function waitForSidebarReady(
  sidebar: DbSaicleGUIWebviewViewProvider,
  timeout: number,
  interval: number,
): Promise<boolean> {
...
const getCommandsMap: (
  ...
  sidebar: DbSaicleGUIWebviewViewProvider,
  consoleView: DbSaicleConsoleWebviewViewProvider,
  ...
  return {
    "dbsaicle.acceptDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "dbsaicle.rejectDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "dbsaicle.defaultQuickAction": async (args: QuickEditShowParams) => {
      captureCommandTelemetry("defaultQuickAction");
      vscode.commands.executeCommand("dbsaicle.focusEdit", args);
    },
    "dbsaicle.customQuickActionSendToChat": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "dbsaicle.customQuickActionStreamInlineEdit": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "dbsaicle.codebaseForceReIndex": async () => {
      core.invoke("index/forceReIndex", undefined);
    },
    "dbsaicle.rebuildCodebaseIndex": async () => {
      core.invoke("index/forceReIndex", { shouldClearIndexes: true });
    },
    "dbsaicle.docsIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: false });
    },
    "dbsaicle.docsReIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: true });
    },
    "dbsaicle.focusDbSaicleInput": async () => {
      const isDbSaicleInputFocused = await sidebar.webviewProtocol.request(
        "isDbSaicleInputFocused",
        undefined,
        false,
      );

      // This is a temporary fixâ€”sidebar.webviewProtocol.request is blocking
      ...
      if (isDbSaicleInputFocused) {
        if (historyLength === 0) {
          hideGUI();
        } else {
          void sidebar.webviewProtocol?.request(
            "focusDbSaicleInputWithNewSession",
            undefined,
            false,
          );
        }
    ...
    "dbsaicle.focusDbSaicleInputWithoutClear": async () => {
      const isDbSaicleInputFocused = await sidebar.webviewProtocol.request(
        "isDbSaicleInputFocused",
        undefined,
        false,
      ...
      if (isDbSaicleInputFocused) {
        ...
        sidebar.webviewProtocol?.request(
          "focusDbSaicleInputWithoutClear",
          undefined,
        );
    ...
    "dbsaicle.writeDocstringForCode": async () => {
    ...
    "dbsaicle.optimizeCode": async () => {
      captureCommandTelemetry("optimizeCode");
      streamInlineEdit("optimize", "Optimize this code");
    },
    "dbsaicle.fixGrammar": async () => {
    ...
    "dbsaicle.clearConsole": async () => {
      consoleView.clearLog();
    },
    "dbsaicle.viewLogs": async () => {
      captureCommandTelemetry("viewLogs");
      vscode.commands.executeCommand("workbench.action.toggleDevTools");
    },
    "dbsaicle.debugTerminal": async () => {
    ...
    "dbsaicle.viewHistory": () => {
      vscode.commands.executeCommand("dbsaicle.navigateTo", "/history", true);
    },
    "dbsaicle.focusDbSaicleSessionId": async (
      ...
      void sidebar.webviewProtocol?.request("focusDbSaicleSessionId", {
        sessionId,
      });
    },
    "dbsaicle.applyCodeFromChat": () => {
      void sidebar.webviewProtocol.request("applyCodeFromChat", undefined);
    },
    "dbsaicle.openConfigPage": () => {
      vscode.commands.executeCommand("dbsaicle.navigateTo", "/config", false);
    },
    "dbsaicle.selectFilesAsContext": async (
      firstUri: vscode.Uri,
      uris: vscode.Uri[],
    ) => {
      if (uris === undefined) {
      ...
      for (const uri of uris) {
        ...
        if (isDirectory) {
          for await (const fileUri of walkDirAsync(uri.toString(), ide, {
            source: "vscode dbsaicle.selectFilesAsContext command",
          })) {
    ...
    "dbsaicle.forceAutocomplete": async () => {
    ...
    "dbsaicle.openTabAutocompleteConfigMenu": async () => {
      captureCommandTelemetry("openTabAutocompleteConfigMenu");

      const config = vscode.workspace.getConfiguration(EXTENSION_NAME);
      const quickPick = vscode.window.createQuickPick();

      const { config: dbsaicleConfig } = await configHandler.loadConfig();
      const autocompleteModels =
        dbsaicleConfig?.modelsByRole.autocomplete ?? [];
      const selected =
        dbsaicleConfig?.selectedModelByRole?.autocomplete?.title ?? undefined;
      ...
      quickPick.onDidAccept(() => {
        ...
        if (targetStatus !== undefined) {
          ...
          vscode.commands.executeCommand("dbsaicle.focusDbSaicleInput");
        } else if (selectedOption === "$(gear) Open settings") {
          vscode.commands.executeCommand("dbsaicle.navigateTo", "/config");
        }
    ...
    "dbsaicle.navigateTo": (path: string, toggle: boolean) => {
      sidebar.webviewProtocol?.request("navigateTo", { path, toggle });
      focusGUI();
    },
    "dbsaicle.startLocalOllama": () => {
      startLocalOllama(ide);
    },
    "dbsaicle.installModel": async (
      modelName: string,
    ...
    "dbsaicle.convertConfigJsonToConfigYaml": async () => {
        ...
        .then(async (selection) => {
          if (selection === "Read the docs") {
            await vscode.env.openExternal(
              vscode.Uri.parse("https://docs.dbsaicle.dev/yaml-migration"),
            );
          }
        });
    },
    "dbsaicle.enterEnterpriseLicenseKey": async () => {
    ...
    "dbsaicle.toggleNextEditEnabled": async () => {
