...
import { DbSaicleCompletionProvider } from "../autocomplete/completionProvider";
import {
...
export class VsCodeExtension {
  ...
  private consoleView: DbSaicleConsoleWebviewViewProvider;
  private sidebar: DbSaicleGUIWebviewViewProvider;
  ...
  private completionProvider: DbSaicleCompletionProvider;
  ...
  private async updateNextEditState(
    ...
    const { config: dbsaicleConfig } = await this.configHandler.loadConfig();
    const autocompleteModel = dbsaicleConfig?.selectedModelByRole.autocomplete;
    ...
    if (
      ...
      !modelSupportsNext &&
      !isNextEditTest() &&
      process.env.DBSAICLE_E2E_NON_NEXT_EDIT_TEST === "true"
    ) {
      vscode.window
        .showWarningMessage(
        ...
        .then((selection) => {
          if (selection === "Disable Next Edit") {
            ...
            vscode.commands.executeCommand(
              "dbsaicle.openTabAutocompleteConfigMenu",
            );
          }
        });
    }
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.browserIntegration = new BrowserIntegration(context);
    this.ide.setBrowserIntegration(this.browserIntegration);
    ...
    this.sidebar = new DbSaicleGUIWebviewViewProvider(
      this.windowId,
    ...
    this.completionProvider = new DbSaicleCompletionProvider(
      this.configHandler,
      this.ide,
      this.sidebar.webviewProtocol,
      usingFullFileDiff,
    ...
    // LLM Log view
    this.consoleView = new DbSaicleConsoleWebviewViewProvider(
      this.windowId,
      this.extensionContext,
      this.core.llmLogger,
    );
    ...
    vscode.authentication.onDidChangeSessions(async (e) => {
      ...
      if (e.provider.id === env.AUTH_TYPE) {
        void vscode.commands.executeCommand(
          "setContext",
          "dbsaicle.isSignedInToControlPlane",
          true,
        );
    ...
    context.subscriptions.push(
      vscode.workspace.registerTextDocumentContentProvider(
        VsCodeExtension.dbsaicleVirtualDocumentScheme,
  ...
  static dbsaicleVirtualDocumentScheme = EXTENSION_NAME;
