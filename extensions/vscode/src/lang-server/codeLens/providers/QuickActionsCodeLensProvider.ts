import { DbSaicleConfig, QuickActionConfig } from "core";
...
export function toggleQuickActions() {
  ...
  getDbSaicleWorkspaceConfig().update(ENABLE_QUICK_ACTIONS_KEY, curStatus);
}

export function quickActionsEnabledStatus() {
  return getDbSaicleWorkspaceConfig().get<boolean>(ENABLE_QUICK_ACTIONS_KEY);
}
...
export class QuickActionsCodeLensProvider implements vscode.CodeLensProvider {
  ...
  getCustomCommands(
    ...
    return quickActionConfigs.map(({ title, prompt, sendToChat }) => {
      return sendToChat
        ? {
            title,
            command: "dbsaicle.customQuickActionSendToChat",
            arguments: [prompt, range],
          }
        : {
            title,
            command: "dbsaicle.customQuickActionStreamInlineEdit",
            arguments: [prompt, range],
          };
    });
  }
  ...
  getDefaultCommand(range: vscode.Range): vscode.Command[] {
    const quickEdit: vscode.Command = {
      command: "dbsaicle.defaultQuickAction",
      title: "DbSaicle",
