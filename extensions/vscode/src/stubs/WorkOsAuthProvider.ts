...
interface DbSaicleAuthenticationSession extends AuthenticationSession {
  refreshToken: string;
  expiresInMs: number;
  loginNeeded: boolean;
}
...
export class WorkOsAuthProvider implements AuthenticationProvider, Disposable {
  ...
  private async storeSessions(value: DbSaicleAuthenticationSession[]) {
  ...
  public async getSessions(
    scopes?: string[],
  ): Promise<DbSaicleAuthenticationSession[]> {
    ...
    try {
      const value = JSON.parse(data) as DbSaicleAuthenticationSession[];
      return value;
    } catch (e: any) {
  ...
  public async createSession(
    ...
    try {
      ...
      if (!token) {
        throw new Error(`DbSaicle login failure`);
      }
      ...
      const session: DbSaicleAuthenticationSession = {
        id: uuidv4(),
  ...
  /**
   * Handle the redirect to VS Code (after sign in from DbSaicle)
   * @param scopes
   * @returns
   */
  private handleUri: (
