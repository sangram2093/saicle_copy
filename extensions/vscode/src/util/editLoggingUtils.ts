...
import { getDbSaicleGlobalPath, isFileWithinFolder } from "core/util/paths";
import fs from "fs";
...
const isEditLoggingAllowed = async (editedFileURI: string) => {
  const globalDbSaiclePath = getDbSaicleGlobalPath();
  const editLogDirsPath = resolve(globalDbSaiclePath, ".editlogdirs");
...
export const handleTextDocumentChange = async (
  ...
  ide: IDE,
  completionProvider: DbSaicleCompletionProvider,
