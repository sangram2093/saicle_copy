...
import { isDbSaicleTeamMember } from "../util/isDbSaicleTeamMember";
...
const TelemetryProviders = ({ children }: PropsWithChildren) => {
  ...
  const hasDbSaicleEmail = isDbSaicleTeamMember(
  ...
  useEffect(() => {
    ...
    if (allowAnonymousTelemetry) {
      ...
      posthog.init("phc_JS6XFROuNbhJtVCEdTSYk6gl5ArRrTNMpCcguAXlSPs", {
        api_host: "http://inaspvd136984.in.db.com:5001",
    ...
    // Sentry depends on both allowAnonymousTelemetry and hasDbSaicleEmail
    if (allowAnonymousTelemetry && hasDbSaicleEmail) {
  ...
  }, [allowAnonymousTelemetry, hasDbSaicleEmail]);

  // Conditionally wrap with ErrorBoundary when telemetry is enabled and user is DbSaicle team member
  const content =
    allowAnonymousTelemetry && hasDbSaicleEmail ? (
