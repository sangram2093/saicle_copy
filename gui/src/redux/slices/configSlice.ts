...
import { BrowserSerializedDbSaicleConfig } from "core";
import { DEFAULT_MAX_TOKENS } from "core/llm/constants";

export type ConfigState = {
  configError: ConfigValidationError[] | undefined;
  config: BrowserSerializedDbSaicleConfig;
  loading: boolean;
};

export const EMPTY_CONFIG: BrowserSerializedDbSaicleConfig = {
  slashCommands: [],
  contextProviders: [],
  tools: [],
  mcpServerStatuses: [],
...
export const configSlice = createSlice({
  ...
  reducers: {
    setConfigResult: (
      ...
      {
        payload: result,
      }: PayloadAction<ConfigResult<BrowserSerializedDbSaicleConfig>>,
    ) => {
    ...
    updateConfig: (
      state,
      { payload: config }: PayloadAction<BrowserSerializedDbSaicleConfig>,
    ) => {
      state.config = config;
    },
