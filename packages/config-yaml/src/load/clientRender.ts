...
function getDbSaicleProxyModelName(
  packageIdentifier: PackageIdentifier,
  provider: string,
  model: string,
): string {
    ...
    for (let i = 0; i < config.models.length; i++) {
      ...
      if (encodedApiKeyLocation || encodedEnvSecretLocations) {
        config.models[i] = {
          ...
          model: getDbSaicleProxyModelName(
            packageIdentifier,
...
/** The additional properties that are added to the otherwise OpenAI-compatible body when requesting a DbSaicle proxy */
export const dbsaiclePropertiesSchema = z.object({
...
export type DbSaicleProperties = z.infer<typeof dbsaiclePropertiesSchema>;
