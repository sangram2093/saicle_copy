...
This SDK provides programmatic access to DbSaicle's Hub APIs and functionality.

## Overview

The DbSaicle SDK is structured into separate language-specific packages, currently with implementations for TypeScript and Python, with more languages planned for the future:
...
1. OpenAPI-generated clients for the DbSaicle Hub API
2. A wrapper layer that exposes a `DbSaicle.from()` method to easily initialize and use DbSaicle assistants with an OpenAI-compatible interface

## For End Users

End users should install the published packages directly:

- For TypeScript/JavaScript: `npm install @dbsaicledev/sdk`
- For Python: `pip install dbsaicledev`
