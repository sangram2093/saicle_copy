...
docker build -t dbsaicle-ubuntu-ssh . # build the image

rm Dockerfile # remove the created Dockerfile

container_name="dbsaicle-ssh-container"

docker run -d -p 2222:22 --name $container_name dbsaicle-ubuntu-ssh # run the container
